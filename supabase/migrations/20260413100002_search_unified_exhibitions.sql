-- Migration: Search Unified Exhibitions
--
...
-- Sprint E-1.2: Add exhibition CTEs to search_unified.
--
-- Adds fts_exhibitions and trgm_exhibitions CTEs so that exhibitions table
-- rows are discoverable through the unified search function. Exhibitions are
-- portal-scoped (ex.portal_id = p_portal_id), filtered by is_active, and
-- respect the same date-range, neighborhood, and free-only guards as events.
-- The 'exhibition' type must appear in p_types for either CTE to emit rows.
--
-- All other CTEs (fts_events, trgm_events, portal_venues, trgm_venues) are
-- preserved verbatim from migration 20260413000010. Signature, DECLARE block,
-- RAISE EXCEPTION guard, STABLE / SECURITY INVOKER, search_path, and
-- statement_timeout are unchanged.
...
COMMENT ON FUNCTION public.search_unified IS
  'Unified search retrieval. Runs FTS + trigram across events, places, and exhibitions as CTEs in one connection. Portal-isolated: events via e.portal_id, places via portal_venues CTE, exhibitions via ex.portal_id. Returns structured venue_name + neighborhood columns. Filters: p_categories, p_neighborhoods, p_date_from, p_date_to, p_free_only (exhibitions: admission_type = ''free''), p_types. 2s statement_timeout. Callable by authenticated + service_role only.';
