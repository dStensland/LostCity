-- Migration: Switch search_events_ranked from subcategory to genres filtering.
-- Adds p_genres parameter (array overlap on e.genres).
-- Keeps p_subcategories in signature (ignored) for backwards compat during rollout.
...
CREATE OR REPLACE FUNCTION search_events_ranked(
    ...
    p_genres TEXT[] DEFAULT NULL          -- NEW: Filter by genres (array overlap)
)
RETURNS TABLE(
    id INTEGER,
    title TEXT,
...
BEGIN
    ...
    WHERE
        ...
        -- Genre filter (replaces old subcategory filter)
...
-- Add GIN index on genres for array overlap queries
CREATE INDEX IF NOT EXISTS idx_events_genres ON events USING GIN(genres) WHERE genres IS NOT NULL;

COMMENT ON FUNCTION search_events_ranked(text, integer, integer, text[], text[], text, boolean, uuid, text[], text[], text[]) IS 'Full-text search for events with relevance ranking. Filters by genres (array overlap). p_subcategories is deprecated and ignored.';
