-- ============================================
-- MIGRATION 361: Diventures Alpharetta Swim Source
-- ============================================
-- Official Diventures Alpharetta recurring swim lesson inventory.
--
-- Portal strategy:
--   Owner:       hooky
--   Subscribers: atlanta
--
-- Pattern role:
--   Swim/location recurring-program implementation backed by public lesson-family
--   cards and class-discovery links.
-- ============================================
...
BEGIN
  ...
  IF atlanta_id IS NULL THEN
    RAISE EXCEPTION 'Atlanta portal not found. Cannot register Diventures source.';
  END IF;
  ...
  SELECT id INTO src_id FROM sources WHERE slug = 'diventures-alpharetta-swim';
  ...
  VALUES (
    ...
    'fitness',
    'https://www.diventures.com/locations/atlanta/swim/',
    'Indoor swim school in Alpharetta offering recurring baby, child, adult, and private swim lessons.'
  )
  ON CONFLICT (slug) DO UPDATE SET
    name = EXCLUDED.name,
