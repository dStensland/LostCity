-- ============================================
-- MIGRATION 285: Interest Channels Atlanta Pilot Seed
-- ============================================
...
BEGIN
  ...
  -- Jurisdiction channel source rules
  ...
  -- School board watch fallback tag rule (until dedicated crawlers are onboarded)
  INSERT INTO interest_channel_rules (channel_id, rule_type, rule_payload, priority, is_active)
  ...
  -- Volunteer channel rules (source + tag)
  INSERT INTO interest_channel_rules (channel_id, rule_type, rule_payload, priority, is_active)
  SELECT c.id, 'source', jsonb_build_object('source_id', s.id, 'source_slug', s.slug), 10, true
  FROM interest_channels c
