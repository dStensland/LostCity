-- Replace Hooky's cheer/gymnastics regex with simple title-term matching.
-- This avoids lacrosse false positives and JS/JSON regex escaping issues.
...
BEGIN
  ...
  IF cheer_channel_id IS NULL THEN
    RAISE NOTICE 'Hooky cheer/gymnastics channel not found. Skipping title-term switch.';
    RETURN;
  END IF;
  ...
  SELECT cheer_channel_id, 'expression', jsonb_build_object(
    'any_title_terms', jsonb_build_array('cheer', 'gymnast', 'tumbling'),
