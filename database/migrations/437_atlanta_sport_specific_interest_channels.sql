...
BEGIN
  ...
  -- Soccer Scene: official match/watch sources plus safe city-core soccer expressions.
  ...
  -- Basketball Scene: official team/watch sources plus Atlanta DPR basketball pickup and March Hoops.
  INSERT INTO interest_channel_rules (channel_id, rule_type, rule_payload, priority, is_active)
  SELECT c.id, 'source', jsonb_build_object('source_id', s.id, 'source_slug', s.slug), 10, true
  FROM interest_channels c
  JOIN sources s ON s.slug IN (
  ...
  -- Racquet Sports: explicit tennis/pickleball sources plus the city-core Piedmont open-play edge.
  INSERT INTO interest_channel_rules (channel_id, rule_type, rule_payload, priority, is_active)
  SELECT c.id, 'source', jsonb_build_object('source_id', s.id, 'source_slug', s.slug), 10, true
  FROM interest_channels c
  JOIN sources s ON s.slug IN (
  ...
  -- Ultimate Frisbee: official Atlanta Hustle plus the recurring Piedmont pickup block.
  INSERT INTO interest_channel_rules (channel_id, rule_type, rule_payload, priority, is_active)
  SELECT c.id, 'source', jsonb_build_object('source_id', s.id, 'source_slug', s.slug), 10, true
  FROM interest_channels c
  JOIN sources s ON s.slug IN (
