-- Migration 607: Align series subscription portal attribution to UUID
--
-- Root cause:
--   Migration 604 created `user_series_subscriptions.portal_id` and
--   `event_rsvps.portal_id` as TEXT. Later schema drift moved
--   `event_rsvps.portal_id` to UUID while `user_series_subscriptions.portal_id`
--   remained TEXT. The auto-RSVP trigger then failed for series-linked event
--   inserts because it copied TEXT portal IDs into a UUID column.
