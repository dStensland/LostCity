-- Fix Hooky's cheer/gymnastics regex for the JavaScript matcher.
-- The interest-channel matcher evaluates title_regex with RegExp in JS,
-- so use \b word boundaries instead of Postgres-specific \m/\M.
...
BEGIN
  ...
  IF cheer_channel_id IS NULL THEN
    RAISE NOTICE 'Hooky cheer/gymnastics channel not found. Skipping regex fix.';
    RETURN;
  END IF;
