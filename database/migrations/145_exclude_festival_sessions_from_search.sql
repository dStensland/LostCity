-- Migration: Exclude festival program sessions from general event search
-- Festival sessions (linked via series with series_type='festival_program') are
-- sub-events of a festival and should only appear on the festival detail page,
-- not in general search results where they create redundancy with the festival itself.

-- Drop old 8-param overload (from migration 049, superseded by 101)
DROP FUNCTION IF EXISTS search_events_ranked(text, integer, integer, text[], text[], text, boolean, uuid);
...
BEGIN
    ...
    WHERE
        ...
        -- Exclude festival program sessions (they belong on the festival page)
        AND NOT EXISTS (
...
COMMENT ON FUNCTION search_events_ranked(text, integer, integer, text[], text[], text, boolean, uuid, text[], text[]) IS 'Full-text search for events with relevance ranking, trigram similarity, portal scoping, subcategory/tag filtering. Excludes festival program sessions.';
