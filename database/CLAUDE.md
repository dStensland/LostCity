...
- **`search_unified()` RPC** (`20260413000007_search_unified.sql`). Single point-of-control for search across events + places + exhibitions. **Always pass `p_portal_id` (NOT NULL, required)** — portal isolation is enforced inside the RPC. Do not write new search queries that bypass this. Required args: `p_portal_id`, `p_query`, `p_types` (e.g. `ARRAY['events','places']`). Optional args: `p_categories`, `p_neighborhoods`, `p_date_from`, `p_date_to`, `p_free_only` (default false), `p_limit_per_retriever` (default 30). Returns tagged rows per retriever for Node-side demultiplexing.
- **`exhibitions` table is first-class, cross-vertical, and mechanically so.** Models persistent experiences at a place — gallery shows, museum exhibitions, aquarium habitats, historic site displays, park attractions — with optional run dates. Schema is portal-agnostic (title, description, place_id, opening_date, closing_date, admission_type, tags, metadata). Has its own `search_vector` (`20260413100001_exhibitions_search_vector.sql`) and is wired into `search_unified()` via exhibition CTEs (commit `bd9cd223`). The `events.exhibition_id` FK shipped 2026-04-14 (`20260413100003_events_exhibition_id.sql`, commit `838b9052`) — associated events (opening nights, artist talks, tours, walkthroughs) set `exhibition_id` to link to the parent exhibition. `content_kind='exhibit'` is **deprecated** (see `crawlers/ARCHITECTURE.md` and commit `89026d9b`); the filter on feed/event queries remains only as protection for legacy rows. **Do not create new `content_kind='exhibit'` rows.** New exhibitions — whether for the Arts, Family, Adventure, or civic portal — go directly in the `exhibitions` table. Note: the `exhibition_type` enum currently carries arts-biased values (`solo/group/installation/retrospective/popup/permanent`) and will be expanded in a follow-up to natively support non-arts types.
