...
- **`venues` → `places` rename** (`20260328200001_places_final_rename.sql`). The table is now `places`. `venue_type` is now `place_type`. `active` is now `is_active`. All foreign keys renamed: `events.venue_id → events.place_id`, `series.venue_id → series.place_id`, etc. In the same migration, `venue_specials` was renamed to `place_specials` and its `venue_id` column became `place_id`. Code is fully migrated; if you find a doc, comment, or migration that still says `venues`, update it.
...
- **`search_unified()` RPC** (`20260413000007_search_unified.sql`, exhibitions added in `20260413100002_search_unified_exhibitions.sql`). Single point-of-control for search across events + places + exhibitions. **Always pass `p_portal_id` (NOT NULL, required)** — portal isolation is enforced inside the RPC. Do not write new search queries that bypass this. Required args: `p_portal_id`, `p_query`, `p_types` (use **singular** values: `ARRAY['event','venue','exhibition']` — note `venue` not `place`; the RPC uses legacy type names internally even though the table renamed to `places`). Optional args: `p_categories`, `p_neighborhoods`, `p_date_from`, `p_date_to`, `p_free_only` (default false), `p_limit_per_retriever` (default 30). Returns tagged rows per retriever for Node-side demultiplexing.
- **`exhibitions` table is first-class, cross-vertical, and mechanically so.** Models persistent experiences at a place — gallery shows, museum exhibitions, aquarium habitats, historic site displays, park attractions — with optional run dates. Schema is portal-agnostic (title, description, place_id, opening_date, closing_date, admission_type, tags, metadata, `related_feature_id` FK to `venue_features`). Has its own `search_vector` (`20260413100001_exhibitions_search_vector.sql`) and is wired into `search_unified()` via exhibition CTEs (commit `bd9cd223`). The `events.exhibition_id` FK shipped 2026-04-14 (`20260413100003_events_exhibition_id.sql`, commit `838b9052`) — associated events (opening nights, artist talks, tours, walkthroughs) set `exhibition_id` to link to the parent exhibition. `content_kind='exhibit'` is **deprecated** (see `crawlers/ARCHITECTURE.md` and commit `89026d9b`); the filter on feed/event queries remains only as protection for legacy rows. **Do not create new `content_kind='exhibit'` rows.** New exhibitions — whether for the Arts, Family, Adventure, or civic portal — go directly in the `exhibitions` table. The `exhibition_type` enum was expanded on 2026-04-10 (`20260410010002_exhibitions_expansion.sql`) and now accepts: `solo`, `group`, `installation`, `retrospective`, `popup`, `permanent`, `seasonal`, `special-exhibit`, `attraction`. Non-arts portals should use `attraction`, `seasonal`, `special-exhibit`, or `permanent`. The `admission_type` enum was also expanded to include `included` (free with venue admission) — values are now: `free`, `ticketed`, `donation`, `suggested`, `included`.
