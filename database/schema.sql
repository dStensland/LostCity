...
CREATE TABLE venues (
  ...
  location geography(POINT, 4326),  -- generated by trigger from lat/lng; requires PostGIS
  venue_type TEXT,
...
-- -------------------------------------------------------
-- Venue Features (persistent attractions/amenities on a place)
-- -------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_venue_features_place ON venue_features(place_id, is_active);
CREATE INDEX IF NOT EXISTS idx_venue_features_source ON venue_features(source_id) WHERE source_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_venue_features_portal ON venue_features(portal_id) WHERE portal_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_exhibitions_feature ON exhibitions(related_feature_id) WHERE related_feature_id IS NOT NULL;
...
-- -------------------------------------------------------
-- Places Refactor (migration 20260328100000)
-- -------------------------------------------------------
...
-- place_candidates: staging for unmatched crawler locations
CREATE TYPE place_candidate_status AS ENUM (
  'pending', 'promoted', 'rejected', 'deferred'
);
...
CREATE INDEX IF NOT EXISTS idx_place_candidates_status
  ON place_candidates(status) WHERE status = 'pending';
...
-- Returns up to 6 mixed items (upcoming events + currently-open places) ranked by temporal relevance.
-- Portal scoping: events filtered by portal_id; places filtered by city only.
-- See: database/migrations/598_right_now_feed_rpc_final.sql
...
-- Screening tables (cinema data model)
