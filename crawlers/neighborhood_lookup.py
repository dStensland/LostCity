"""
Shared neighborhood inference from lat/lng coordinates.

Used by:
- db/venues.py  (auto-populate neighborhood on new venue creation)
- assign_neighborhoods.py  (batch backfill script)

Neighborhood zones: (name, centroid_lat, centroid_lng, radius_meters)
The radius represents the approximate catchment area for that neighborhood.
When multiple zones overlap, the one with the lowest distance/radius ratio wins
(i.e. the zone where the point is most "centrally" located).
"""
...
NEIGHBORHOODS_BY_CITY: dict[str, list[tuple[str, float, float, float]]] = {
    "Atlanta": [
        # ── ITP ──
        ...
        ("Cheshire Bridge", 33.810, -84.360, 1200),
        ("Downtown Decatur", 33.7748, -84.2963, 800),
        ("Emory", 33.7925, -84.3230, 1500),
        ("Peachtree Hills", 33.8180, -84.3740, 800),
        ("Carey Park", 33.7000, -84.3710, 800),
        # ── OTP / Suburbs ──
        ...
        ("Marietta Square", 33.9527, -84.5502, 800),
        ("Cumberland", 33.8810, -84.4690, 2000),
        ("South Fulton", 33.6300, -84.5700, 5000),
...
# Backward-compatible flat list — used by assign_neighborhoods.py
NEIGHBORHOODS: list[tuple[str, float, float, float]] = []
for _city_hoods in NEIGHBORHOODS_BY_CITY.values():
    NEIGHBORHOODS.extend(_city_hoods)

# Normalization aliases — canonical form → variants found in DB
...
def infer_neighborhood_from_coords(
    lat: float, lng: float, city: str = "Atlanta"
) -> Optional[str]:
    """
    Return the best-matching neighborhood name for the given coordinates, or None.

    Scoped to the given city's neighborhood table. Falls back to Atlanta if the
    city isn't in our lookup.

    Matching logic: the point must fall within a zone's radius.  When multiple
    zones match, the one with the lowest distance/radius ratio wins (most central).
    This keeps smaller, denser ITP neighborhoods winning over large suburb zones
    when the point sits inside both catchment areas.
    """
    neighborhoods = NEIGHBORHOODS_BY_CITY.get(city, NEIGHBORHOODS_BY_CITY.get("Atlanta", []))
