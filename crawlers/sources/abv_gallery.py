...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl ABV Gallery events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching ABV Gallery: {EVENTS_URL}")
            ...
            while i < len(lines):
                ...
                if date_match:
                    ...
                    event_record = {
                        ...
                        "venue_id": venue_id,
                        "title": title,
                        "description": "Event at ABV Gallery",
        ...
        logger.info(
            f"ABV Gallery crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl ABV Gallery: {e}")
        raise
