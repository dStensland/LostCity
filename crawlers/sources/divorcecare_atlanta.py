"""
Crawler for DivorceCare divorce recovery support groups in Atlanta metro area.
https://find.divorcecare.org - national directory with location search

DivorceCare is a church-based divorce recovery program (13-week cycles).
Category: wellness
...
BASE_URL = "https://find.divorcecare.org"
SEARCH_URL = "https://www.divorcecare.org/findagroup"
...
def parse_date_string(date_str: str) -> Optional[str]:
    """
    Parse date from DivorceCare format.
    Examples:
      - "Sunday, February 15"
...
def parse_time_range(time_str: str) -> tuple[Optional[str], Optional[str]]:
    """
    Parse time range from DivorceCare format.
    Examples:
      - "11:15am–12:45pm ET"
      - "7–8:30pm ET"
      - "6:30–8:30pm ET"
...
def parse_groups_from_text(text_lines: list[str]) -> list[dict]:
    ...
    while i < len(text_lines):
        ...
        # Look for "DIVORCECARE GROUP" marker
        if "DIVORCECARE GROUP" in line.upper():
            try:
                group_data = {}
                i += 1
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl DivorceCare directory for Atlanta metro area groups."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            for zip_code in ATLANTA_ZIPS[:5]:  # Limit to 5 searches to avoid overload
                logger.info(f"Searching DivorceCare for zip: {zip_code}")
        ...
        for group in all_groups:
            try:
                ...
                # Build tags
                tags = ["support-group", "divorce", "faith-based", "free"]
                if has_childcare:
                ...
                # Build description
                description_parts = [
                    f"A 13-week DivorceCare support group meeting {day_of_week}s at {church_name}."
                ]
                if has_childcare:
        ...
        logger.info(
            f"DivorceCare crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl DivorceCare: {e}")
        raise
