"""
Crawler for DeKalb County Midway pickleball open play.

Targets the official Midway pickleball listing in DeKalb's ACTIVENet catalog and
expands the published Monday schedule into recurring open-play instances.
"""
...
SCHEDULE_RE = re.compile(
    r"(?P<days>Mondays?)\s+(?P<start>\d{1,2}:\d{2}\s*(?:am|pm))\s*[–-]\s*(?P<end>\d{1,2}:\d{2}\s*(?:am|pm))",
    re.IGNORECASE,
)
...
def is_target_item(item: dict) -> bool:
    return (item.get("name") or "").strip().lower() == TARGET_NAME.lower()
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not session or not csrf:
        logger.error("DeKalb Midway pickleball crawl aborted: session init failed")
        return 0, 0, 0

    first = _fetch_page(session, csrf, 1)
    if first is None:
        logger.error("DeKalb Midway pickleball crawl aborted: failed to fetch page 1")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info("Removed %s stale DeKalb Midway pickleball rows", stale_removed)

    logger.info(
        "DeKalb Midway pickleball crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
