"""
Crawler for Cobb County public belly dance classes.

Targets the official Rec1 belly dance group and expands the county's
...
PLACE_DATA = {
    ...
    "website": CATALOG_URL,
    "description": "Cobb County recreation center hosting public belly dance classes.",
}
...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    title = f"Belly Dance at {PLACE_DATA['name']}"
    description = (
        f"Public belly dance class at {PLACE_DATA['name']} through Cobb County Parks. "
    ...
    return {
        ...
        "raw_text": (
            f"Belly Dance | {features.get('days', '')} | "
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not checkout_key:
        logger.error("Cobb belly dance crawl aborted: missing checkout key")
        return 0, 0, 0

    tabs = _get_tabs(TENANT_SLUG, checkout_key)
    if not any(str(tab.get("id")) == TARGET_TAB_ID for tab in tabs):
        logger.error("Cobb belly dance crawl aborted: target tab missing")
        return 0, 0, 0
    ...
    if not target_group:
        logger.error("Cobb belly dance crawl aborted: target group missing")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info("Removed %s stale Cobb belly dance rows", stale_removed)

    logger.info(
        "Cobb belly dance crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
