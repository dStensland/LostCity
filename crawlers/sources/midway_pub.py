...
East Atlanta Village neighborhood pub.
...
WEEKLY_SCHEDULE = [
    {
        "day": 2,  # Wednesday
        "title": "Trivia at Midway Pub",
        "description": (
            "Wednesday trivia night at Midway Pub in East Atlanta Village. "
            "Neighborhood pub trivia with cheap drinks and good vibes."
        ),
        "start_time": "20:00",
        "category": "nightlife",
        "subcategory": "nightlife.trivia",
        "tags": ["trivia", "nightlife", "weekly", "east-atlanta"],
    },
]
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Ensure Midway Pub venue and generate recurring trivia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    for template in WEEKLY_SCHEDULE:
        ...
        for week in range(WEEKS_AHEAD):
            ...
            event_record = {
                ...
                "ticket_url": None,
                "image_url": None,
                "raw_text": f"{template['title']} at Midway Pub - {start_date}",
    ...
    logger.info(
        f"Midway Pub crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
    )
