"""
Crawler for Wylie & Rum (wylieandrum.com).
Caribbean-inspired bar in Reynoldstown near the BeltLine.

Generates recurring Wednesday trivia events (Rum Punch Wednesday).
"""
...
PLACE_DATA = {
    ...
    "spot_type": "bar",
    "website": BASE_URL,
    "vibes": ["caribbean", "rum", "cocktails", "reynoldstown", "beltline", "patio"],
}

WEEKLY_SCHEDULE = [
    {
        "day": 2,  # Wednesday
        "title": "Trivia at Wylie & Rum",
        "description": (
            "Wednesday trivia night at Wylie & Rum in Reynoldstown. "
            "Rum Punch Wednesday — trivia from 7-9pm with prizes ($30/$20/$10) "
            "and Caribbean cocktails near the BeltLine."
        ),
        "start_time": "19:00",
        "category": "nightlife",
        "subcategory": "nightlife.trivia",
        "tags": ["trivia", "nightlife", "weekly", "reynoldstown", "beltline", "rum"],
    },
]
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Ensure Wylie & Rum venue and generate recurring trivia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    logger.info(
        f"Wylie & Rum crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
    )
