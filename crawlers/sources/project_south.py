"""
Crawler for Project South (projectsouth.org).

Project South is a social justice institute founded in 1986, focusing on
leadership development, education, and community organizing.
...
BASE_URL = "https://projectsouth.org"
EVENTS_URL = f"{BASE_URL}/events"

# Project South HQ venue
PROJECT_SOUTH_HQ = {
    "name": "Project South",
    "slug": "project-south",
    "address": "9 Gammon Ave SW",
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["workshop", "training", "education", "seminar", "class", "learn"]):
        return "education"
    if any(word in text for word in ["town hall", "community meeting", "forum", "discussion", "assembly"]):
        return "community"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    if any(word in text for word in ["economic", "labor", "worker", "employment"]):
        tags.append("economic-justice")
    if any(word in text for word in ["environment", "climate", "environmental justice"]):
        tags.append("environmental-justice")
    ...
    if any(word in text for word in ["race", "racial", "racism", "antiracism"]):
        tags.append("racial-justice")

    # Add free tag (most organizing events are free)
    if "free" in text or "volunteer" in tags or "workshop" in text:
        tags.append("free")
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    # Default to True for social justice org (most events are free)
    return True


def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Project South events using Playwright.
    ...
    try:
        with sync_playwright() as p:
            ...
            # Get venue ID for Project South HQ
            venue_id = get_or_create_venue(PROJECT_SOUTH_HQ)

            logger.info(f"Fetching Project South events: {EVENTS_URL}")
                    ...
                    for event_elem in event_elements:
                        try:
                            ...
                            content_hash = generate_content_hash(
                                title, "Project South", start_date
                            )
                ...
                while i < len(lines):
                    ...
                    if parsed_date and i + 1 < len(lines):
                        ...
                        if len(potential_title) > 10 and not potential_title.startswith("http"):
                            ...
                            content_hash = generate_content_hash(
                                title, "Project South", parsed_date
                            )
        ...
        logger.info(
            f"Project South crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Project South: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Project South: {e}")
        raise
