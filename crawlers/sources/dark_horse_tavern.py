...
WEEKLY_SCHEDULE = [
    {
        "day": 2,  # Wednesday
        "title": "Trivia at Dark Horse Tavern",
        "description": (
            "Wednesday trivia night at Dark Horse Tavern in Virginia-Highland. "
            "Weekly pub trivia with wings and cold beers at this neighborhood staple."
        ),
        "start_time": "20:00",
        "category": "nightlife",
        "subcategory": "nightlife.trivia",
        "tags": ["trivia", "nightlife", "weekly", "sports-bar"],
    },
]
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Ensure Dark Horse Tavern venue and generate recurring trivia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    for template in WEEKLY_SCHEDULE:
        ...
        for week in range(WEEKS_AHEAD):
            ...
            event_record = {
                ...
                "ticket_url": None,
                "image_url": None,
                "raw_text": f"{template['title']} at Dark Horse Tavern - {start_date}",
    ...
    logger.info(
        f"Dark Horse Tavern crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
    )
