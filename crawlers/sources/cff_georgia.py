"""
Crawler for Cystic Fibrosis Foundation Georgia (cff.org/chapters/georgia-chapter).

The CFF Georgia Chapter provides support and advocacy for people with
cystic fibrosis and their families.

Events include:
- Great Strides Walk (major annual walk)
- Wine & Roses Gala (signature fundraiser)
- NACFC Conference (North American Cystic Fibrosis Conference) - October 2026 in Atlanta!
- Educational programs and support groups

Located in Briarcliff area.
"""
...
VENUE_DATA = {
    ...
    "website": GEORGIA_URL,
    "vibes": ["cystic-fibrosis", "respiratory", "support", "research"],
}
...
def categorize_event(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    # Great Strides Walk
    if any(word in text for word in ["great strides", "walk", "5k", "fundraiser"]):
        category = "community"
        tags.extend(["fundraiser", "walk", "outdoor"])
        is_free = "free registration" in text

    # Wine & Roses Gala
    elif any(word in text for word in ["wine", "roses", "gala", "benefit"]):
        category = "community"
        tags.extend(["fundraiser", "gala", "formal"])
        is_free = False

    # NACFC Conference
    elif any(word in text for word in ["nacfc", "conference", "north american cystic fibrosis"]):
        category = "learning"
        tags.extend(["conference", "medical", "research"])
        is_free = False

    # Support groups
    elif any(word in text for word in ["support group", "patient support", "family support"]):
        category = "wellness"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Cystic Fibrosis Foundation Georgia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        if response.status_code != 200:
            logger.info(f"CFF Georgia venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for container in event_containers:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "Cystic Fibrosis Foundation Georgia", start_date
                )
        ...
        logger.info(
            f"CFF Georgia crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl CFF Georgia: {e}")
        raise
