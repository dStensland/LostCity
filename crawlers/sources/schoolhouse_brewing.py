"""
Crawler for Schoolhouse Brewing (schoolhousebrewing.com).

Brewery in Marietta with live music and community events.
...
BASE_URL = "https://www.schoolhousebeer.com"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Schoolhouse Brewing",
    "slug": "schoolhouse-brewing",
    "address": "225 Lawrence St NE",
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Schoolhouse Brewing events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Schoolhouse Brewing: {EVENTS_URL}")
            ...
            while i < len(lines):
                ...
                if date_match:
                    ...
                    content_hash = generate_content_hash(
                        title, "Schoolhouse Brewing", start_date
                    )
        ...
        logger.info(
            f"Schoolhouse Brewing crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl Schoolhouse Brewing: {e}")
        raise
