"""
Crawler for Center for Endometriosis Care Atlanta (centerforendo.com).

The Center for Endometriosis Care is a specialized treatment center providing:
- Patient education on endometriosis
- Awareness events and campaigns
- Support resources for women with endometriosis
- Clinical care from excision specialists

Located in Sandy Springs, GA, the center has been treating endometriosis for 35+ years.

STRATEGY:
- Scrape events/news page for patient education and awareness events
- Extract webinars, seminars, and community events
- Tag: endometriosis, womens-health, education
- Category: "learning" or "community"
- Some events may be free, others may require registration
"""
...
BASE_URL = "https://www.centerforendo.com"
EVENTS_URL = f"{BASE_URL}"

VENUE_DATA = {
    "name": "Center for Endometriosis Care Atlanta",
    "slug": "center-for-endometriosis-care-atlanta",
    "address": "6105 Peachtree Dunwoody Rd",
    ...
    "website": BASE_URL,
    "vibes": ["endometriosis", "womens-health", "treatment"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["endometriosis", "womens-health"]

    # Educational events
    if any(word in text for word in ["webinar", "seminar", "workshop", "education", "class", "presentation"]):
        category = "learning"
        tags.append("education")
    # Support groups
    elif any(word in text for word in ["support", "group", "meeting"]):
        category = "support_group"
        tags.append("support-group")
    # Awareness/advocacy
    elif any(word in text for word in ["awareness", "campaign", "advocacy", "march"]):
        category = "community"
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|news|endometriosis', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Center for Endometriosis Care Atlanta events.
    """
    source_id = source["id"]
    events_found = 0
    events_new = 0
    ...
    try:
        ...
        logger.info(f"Fetching Center for Endometriosis Care events: {EVENTS_URL}")
        soup = try_simple_requests_first(EVENTS_URL)

        if not soup:
            logger.info("No event-related content found; ensuring venue record only")
            logger.info(f"Center for Endometriosis Care venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                # Extract description
                description = None
                desc_elem = event_elem.select_one(".description, .excerpt, .summary, p")
                if desc_elem:
                ...
                content_hash = generate_content_hash(
                    title, "Center for Endometriosis Care Atlanta", start_date
                )
        ...
        logger.info(
            f"Center for Endometriosis Care crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl Center for Endometriosis Care: {e}")
        raise
