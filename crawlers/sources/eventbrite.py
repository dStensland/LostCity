...
BROWSE_URLS = [
    ...
    # charity-and-causes removed — civic categories handled by eventbrite-civic source (HelpATL)
...
def enrich_description_from_detail_page(
    base_description: Optional[str], event_url: str
) -> str:
...
def build_structured_eventbrite_description(
    *,
    ...
    if event_url:
        parts.append(
            f"Check Eventbrite for current agenda, policy updates, and ticket availability ({event_url})."
        )
...
def discover_event_ids(max_events: int = 500) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        for browse_url in BROWSE_URLS:
            ...
            while scroll_count < max_scrolls and len(event_ids) < max_events:
                ...
                logger.info(
                    f"Scroll {scroll_count + 1}: Found {len(event_ids)} unique events so far"
                )
    ...
    logger.info(
        f"Discovered {len(event_ids)} unique event IDs across {len(BROWSE_URLS)} category pages"
    )
...
def process_event(
    event_data: dict, source_id: int, producer_id: Optional[int]
) -> Optional[dict]:
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    try:
        ...
        for i, event_id in enumerate(event_ids):
            if i > 0 and i % 50 == 0:
                logger.info(
                    f"Progress: {i}/{len(event_ids)} events processed, {events_new} new"
                )
            ...
            try:
                ...
                events_new += 1
                logger.debug(
                    f"Added: {result['title'][:50]}... on {result['start_date']}"
                )
            except Exception as e:
