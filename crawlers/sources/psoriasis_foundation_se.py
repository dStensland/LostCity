"""
Crawler for National Psoriasis Foundation Southeast (psoriasis.org).

The National Psoriasis Foundation provides support, education, and advocacy
for people living with psoriasis and psoriatic arthritis. The Southeast chapter
hosts fundraising events, support groups, and educational programs.

Key Events:
- Southeast Soirée "Jazz & Generosity" annual gala at Ventanas Atlanta
- Patient conferences and educational seminars
- Support groups and community programs

Relevant for hospital patients and caregivers dealing with autoimmune conditions.

STRATEGY:
- Scrape the events/calendar page
- Extract fundraising galas, patient education events, support groups
- Tag with: psoriasis, skin-disease, autoimmune, patient-support
- Most events are free except fundraisers (gala tickets)
- Category: "community" for galas/fundraisers, "learning" for education events
"""
...
BASE_URL = "https://www.psoriasis.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "National Psoriasis Foundation Southeast",
    "slug": "psoriasis-foundation-southeast",
    "address": "125 Park Place NE",
    ...
    "website": BASE_URL,
    "vibes": ["psoriasis", "skin-disease", "autoimmune", "patient-support"],
}
...
def parse_time_string(time_str: str) -> Optional[str]:
    ...
    Examples: '6:00 PM', '12:30 PM', '6pm', '6:30 PM ET'
    """
    try:
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["psoriasis", "autoimmune"]

    # Fundraising galas and soirées
    if any(word in text for word in ["gala", "soirée", "soiree", "fundraiser", "benefit", "jazz & generosity"]):
        category = "community"
        tags.extend(["fundraiser", "gala"])
        is_free = False
    ...
    # Awareness events
        ...
        category = "fitness"
        tags.extend(["awareness", "fundraiser"])
        is_free = False
    ...
    if any(word in text for word in ["free", "no cost", "complimentary"]):
        is_free = True
        tags.append("free")
    elif any(word in text for word in ["$", "ticket", "registration fee", "donation"]):
        # Already marked as not free for galas/fundraisers
        pass

    # Add skin disease tag
    tags.append("skin-disease")
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|calendar|conference|gala', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl National Psoriasis Foundation Southeast events.
    ...
    try:
        ...
        # If no structured events found, log and ensure venue exists
        if not events or len(events) == 0:
            logger.info("No structured event elements found")
            logger.info(f"National Psoriasis Foundation Southeast venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        logger.info(
            f"National Psoriasis Foundation Southeast crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Psoriasis Foundation events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl National Psoriasis Foundation Southeast: {e}")
        raise
