"""
Crawler for Best Buddies Georgia (bestbuddies.org/georgia).

Best Buddies Georgia creates opportunities for people with intellectual and
developmental disabilities. Events include:
- Friendship Walk (April 26, 2026)
- Employment Breakfast (March 12)
...
BASE_URL = "https://www.bestbuddies.org/georgia"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Best Buddies Georgia",
    "slug": "best-buddies-georgia",
    "address": "1201 W Peachtree St NW, Suite 3000",
    ...
    "website": BASE_URL,
    "vibes": ["disabilities", "inclusion", "friendship", "community"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["walk", "friendship walk"]):
        return "fitness"
    if any(word in text for word in ["gala", "champion"]):
        return "fundraiser"
    if any(word in text for word in ["breakfast", "luncheon"]):
        return "fundraiser"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["disabilities", "inclusion"]

    if any(word in text for word in ["friendship"]):
        tags.append("friendship")
    if any(word in text for word in ["walk", "run"]):
        tags.append("walk")
    if any(word in text for word in ["fundraiser", "gala"]):
        tags.append("fundraiser")
    if any(word in text for word in ["employment", "jobs"]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Best Buddies Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Best Buddies Georgia events: {EVENTS_URL}")
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Best Buddies Georgia", start_date
                        )
                        ...
                        if link_elem:
                            ...
                            if href:
                                if href.startswith("http"):
                                    ...
                                    event_url = BASE_URL.replace("/georgia", "") + href
        ...
        logger.info(
            f"Best Buddies Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Best Buddies Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Best Buddies Georgia: {e}")
        raise
