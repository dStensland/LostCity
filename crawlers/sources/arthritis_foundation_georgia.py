"""
Crawler for Arthritis Foundation Georgia (arthritis.org/georgia).

The Arthritis Foundation NATIONAL HEADQUARTERS is in Midtown Atlanta.
They provide support, research, and advocacy for arthritis and rheumatic diseases.

Events include:
- Walk to Cure Arthritis (celebrating 25th anniversary in 2026)
- Jingle Bell Run (holiday fundraiser)
- Connect Groups (support groups)
- Educational workshops

This is the NATIONAL headquarters, so events here are significant.
"""
...
BASE_URL = "https://www.arthritis.org"
GEORGIA_URL = f"{BASE_URL}/georgia"
EVENTS_URL = f"{GEORGIA_URL}/events"

VENUE_DATA = {
    "name": "Arthritis Foundation Georgia",
    "slug": "arthritis-foundation-georgia",
    "address": "1355 Peachtree St NE, Suite 600",
    ...
    "website": GEORGIA_URL,
    "vibes": ["arthritis", "rheumatic", "support", "national-hq"],
}
...
def categorize_event(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["arthritis", "rheumatic"]

    # Walk to Cure Arthritis
    if any(word in text for word in ["walk to cure", "walk", "5k", "fundraiser"]):
        category = "community"
        tags.extend(["fundraiser", "walk", "outdoor"])
        is_free = "free registration" in text

    # Jingle Bell Run
    elif any(word in text for word in ["jingle bell", "run", "holiday"]):
        category = "community"
        tags.extend(["fundraiser", "5k", "holiday", "outdoor"])
        is_free = False

    # Connect Groups (support groups)
    elif any(word in text for word in ["connect group", "support group", "peer support"]):
        category = "wellness"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Arthritis Foundation Georgia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        logger.info(f"Fetching Arthritis Foundation events: {EVENTS_URL}")
        ...
        if response.status_code != 200:
            logger.info(f"Arthritis Foundation venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for container in event_containers:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "Arthritis Foundation Georgia", start_date
                )
        ...
        logger.info(
            f"Arthritis Foundation crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl Arthritis Foundation: {e}")
        raise
