"""
Crawler for Atlanta Black Pride (atlblackpride.org).
    ...
    try:
        ...
        for candidate_url in FALLBACK_URLS:
            try:
                ...
                resp = requests.get(candidate_url, headers=headers, timeout=30, allow_redirects=True)
                final_url = resp.url
        ...
        if not selected_url or soup is None:
            logger.warning(
        ...
        logger.error(f"Failed to crawl Atlanta Black Pride: {e}")
        raise
