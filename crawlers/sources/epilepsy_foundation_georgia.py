"""
Crawler for Epilepsy Foundation of Georgia (epilepsyga.org).

Epilepsy Foundation of Georgia provides support, education, and advocacy for individuals
with epilepsy and their families.

Events include:
- Magnolia Run/Walk (annual fundraiser)
...
Highly relevant for hospital portal (neurological care, family support, education).
"""
...
BASE_URL = "https://epilepsyga.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Epilepsy Foundation of Georgia",
    "slug": "epilepsy-foundation-georgia",
    "address": "6065 Roswell Rd NE, Suite 615",
    ...
    "website": BASE_URL,
    "vibes": ["epilepsy", "neurological", "support"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["magnolia run", "walk", "run", "5k"]):
        return "fitness"

    if any(word in text for word in ["camp", "summer camp"]):
        return "kids"

    if any(word in text for word in ["workshop", "seminar", "training", "education", "first aid"]):
        return "learning"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["epilepsy", "seizure", "neurological"]
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    # Support groups and camps are typically free
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Epilepsy Foundation of Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Epilepsy Foundation Georgia events: {EVENTS_URL}")
            ...
            for event_elem in events:
                try:
                    ...
                    content_hash = generate_content_hash(title, "Epilepsy Foundation Georgia", start_date)
        ...
        logger.info(f"Epilepsy Foundation Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated")

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Epilepsy Foundation Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Epilepsy Foundation Georgia: {e}")
        raise
