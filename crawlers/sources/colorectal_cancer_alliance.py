"""
Crawler for Colorectal Cancer Alliance (colorectalcancer.org).

The Colorectal Cancer Alliance is the leading nonprofit dedicated to ending
colorectal cancer through patient support, awareness programs, and screening advocacy.

Key Events:
- Walk to End Colon Cancer Atlanta
- Blue Hope Bash (annual gala)
- Patient education programs
- Awareness and screening events

Relevant for hospital patients, at-risk populations, and caregivers.

STRATEGY:
- Scrape the events page for Atlanta-area events
- Extract awareness walks, patient programs, screening events
- Tag with: colorectal-cancer, awareness, screening, patient-support
- Most educational/awareness events are free, fundraisers are ticketed
- Category: "fitness" for walks, "learning" for education, "community" for awareness
"""
...
BASE_URL = "https://colorectalcancer.org"
EVENTS_URL = f"{BASE_URL}/get-involved/attend-an-event/"

VENUE_DATA = {
    "name": "Colorectal Cancer Alliance",
    "slug": "colorectal-cancer-alliance",
    ...
    "website": BASE_URL,
    "vibes": ["colorectal-cancer", "awareness", "screening", "patient-support"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    if any(word in text for word in ["walk to end colon cancer", "walk", "5k", "run"]):
        category = "fitness"
        tags.extend(["awareness", "walk", "fundraiser"])
        is_free = False  # Walk typically has registration fee

    # Blue Hope Bash and fundraising galas
    elif any(word in text for word in ["blue hope bash", "gala", "benefit", "fundraiser"]):
        category = "community"
        tags.extend(["fundraiser", "gala"])
        is_free = False

    # Patient education and support
    elif any(word in text for word in ["patient", "support group", "education", "seminar", "workshop"]):
        category = "learning"
        tags.extend(["patient-support", "patient-education"])
        is_free = "free" in text or "no cost" in text

    # Screening events
    elif any(word in text for word in ["screening", "colonoscopy", "awareness"]):
        category = "community"
        tags.extend(["screening", "awareness", "preventive-care"])
        is_free = "free" in text or "no cost" in text
    ...
    if any(word in text for word in ["free", "no cost", "complimentary"]):
        ...
        if "free registration" not in text:
            is_free = False
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|calendar|walk|screening', re.I)):
            return soup
    ...
    try:
        ...
        for event_elem in events:
            try:
                ...
                event_text = event_elem.get_text().lower()
                if not any(loc in event_text for loc in ["atlanta", "georgia", "ga"]):
                ...
                content_hash = generate_content_hash(
                    title, "Colorectal Cancer Alliance", start_date
                )
        ...
        logger.info(
            f"Colorectal Cancer Alliance crawl complete: {events_found} found, "
