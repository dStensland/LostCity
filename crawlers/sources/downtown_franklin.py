"""
Crawler for Downtown Franklin Association (downtownfranklintn.com/events).
Historic downtown district - community events, arts, and festivals.
...
BASE_URL = "https://downtownfranklintn.com"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Downtown Franklin Association events using Playwright.
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Downtown Franklin events: {EVENTS_URL}")
            ...
            logger.info(
                f"Found {len(cards)} potential event cards on Downtown Franklin"
            )
            ...
            for card in cards:
                try:
                    ...
                    event_record = {
                        ...
                        "category": category,
                        "subcategory": None,
                        "tags": ["franklin", "downtown-franklin"],
                        "price_min": None,
                        "price_max": None,
                        "price_note": None,
                        "is_free": False,
        ...
        logger.info(
            f"Downtown Franklin crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Downtown Franklin: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Downtown Franklin: {e}")
        raise
