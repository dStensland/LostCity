"""
Crawler for AdventHealth Georgia Community Events.

AdventHealth has a regional presence in Northwest Georgia (Gordon/Calhoun and
Murray/Chatsworth areas). Events include health screenings, support groups,
nutrition/fitness classes, lectures, community festivals, 5Ks, and wellness programs.

Limited direct Atlanta metro presence but valuable for regional health coverage
and community wellness programming in the broader Atlanta metro area.

STRATEGY:
- Scrape events page at adventhealth.com/events (filtered by Georgia locations)
- Uses Drupal-based event system
- Extract community health events, screenings, wellness programs, fitness events
- Tag: community-health, hospital, wellness, fitness
- Mix of free and paid events
- Category: "community" for health fairs/events, "education" for classes, "sports" for 5Ks

Relevant for Emory portal's community health track and regional coverage.
"""
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    # 5K runs and fitness events
    if any(word in text for word in ["5k", "run", "walk", "race", "marathon", "fun run"]):
        category = "sports"
        tags.extend(["fitness", "wellness", "5k", "running"])
    ...
    elif any(word in text for word in ["fitness", "exercise", "yoga", "zumba", "pilates", "workout"]):
        category = "wellness"
        tags.extend(["fitness", "exercise"])

    # Nutrition classes
    elif any(word in text for word in ["nutrition", "cooking", "diet", "healthy eating", "weight loss"]):
        category = "learning"
        tags.extend(["nutrition", "wellness", "health-education"])

    # Health education workshops and classes
    elif any(word in text for word in ["workshop", "class", "education", "training", "seminar", "lunch and learn"]):
        category = "learning"
        tags.extend(["health-education", "workshop", "wellness"])

    # Community events and fundraisers
    elif any(word in text for word in ["fundraiser", "gala", "benefit", "community event", "festival", "celebration"]):
        category = "community"
    ...
    # Lectures and talks
        ...
        category = "learning"
        tags.extend(["lecture", "health-education"])
    ...
    if any(word in text for word in ["free", "no cost", "no charge", "complimentary", "no fee"]):
        is_free = True
        tags.append("free")
    elif any(word in text for word in ["$", "registration fee", "cost", "ticket", "donation"]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl AdventHealth Georgia events.
    ...
    try:
        ...
        event_selectors = [
            ...
            ".node--type-event",  # Drupal content type
            ".paragraph--type-event",  # Drupal paragraphs
        ]
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"AdventHealth Gordon venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "AdventHealth Gordon", start_date
                )
        ...
        logger.info(
            f"AdventHealth Georgia crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching AdventHealth events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl AdventHealth Georgia: {e}")
        raise
