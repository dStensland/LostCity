...
Little Five Points restaurant and bar.
...
PLACE_DATA = {
    ...
    "spot_type": "bar",
    "website": BASE_URL,
    "vibes": ["dive-bar", "late-night", "cheap-drinks", "little-five-points"],
}

WEEKLY_SCHEDULE = [
    {
        "day": 2,  # Wednesday
        "title": "Trivia at Elmyr",
        "description": (
            "Wednesday trivia night at Elmyr in Little Five Points. "
            "Pub trivia with cocktails and small plates in the heart of L5P."
        ),
        "start_time": "20:00",
        "category": "nightlife",
        "subcategory": "nightlife.trivia",
        "tags": ["trivia", "nightlife", "weekly"],
    },
]
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Ensure Elmyr venue and generate recurring trivia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    for template in WEEKLY_SCHEDULE:
        ...
        for week in range(WEEKS_AHEAD):
            ...
            event_record = {
                ...
                "ticket_url": None,
                "image_url": None,
                "raw_text": f"{template['title']} at Elmyr - {start_date}",
    ...
    logger.info(
        f"Elmyr crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
    )
