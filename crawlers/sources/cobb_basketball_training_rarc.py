...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    description = build_program_description(
        title,
        summary="Public youth basketball training through Cobb County Parks.",
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not checkout_key:
        logger.error(
            "Cobb basketball training RARC crawl aborted: missing checkout key"
        )
        return 0, 0, 0
    ...
    if not target_group:
        logger.error(
            "Cobb basketball training RARC crawl aborted: target group missing"
        )
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info(
            "Removed %s stale Cobb basketball training RARC rows", stale_removed
        )
