"""
Crawler for Cobb County youth basketball training at Ron Anderson.

Targets the official Rec1 RARC basketball training group and expands the
published weekday/date-range schedule into recurring youth class instances.
"""
...
TARGET_TAB_ID = "8572"
TARGET_GROUP = "Basketball Training - RARC"
WEEKS_AHEAD = 10
...
PLACE_DATA = {
    "name": "Ron Anderson Recreation Center",
    "slug": "ron-anderson-recreation-center",
    "address": "3820 Macedonia Rd",
    ...
    "website": CATALOG_URL,
    "description": "Cobb County recreation center hosting public youth basketball training.",
}
...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    if location != "ron anderson recreation center":
        return None
    ...
    title = f"Youth Basketball Training at {PLACE_DATA['name']}"
    description = (
        f"Public youth basketball training at {PLACE_DATA['name']} through Cobb County Parks. "
    ...
    return {
        ...
        "raw_text": (
            f"Basketball Training - RARC | {features.get('ageGender', '')} | "
            f"{features.get('days', '')} | {features.get('dates', '')} | "
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not checkout_key:
        logger.error("Cobb basketball training RARC crawl aborted: missing checkout key")
        return 0, 0, 0

    tabs = _get_tabs(TENANT_SLUG, checkout_key)
    if not any(str(tab.get("id")) == TARGET_TAB_ID for tab in tabs):
        logger.error("Cobb basketball training RARC crawl aborted: target tab missing")
        return 0, 0, 0
    ...
    if not target_group:
        logger.error("Cobb basketball training RARC crawl aborted: target group missing")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info("Removed %s stale Cobb basketball training RARC rows", stale_removed)

    logger.info(
        "Cobb basketball training RARC crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
