"""
Crawler for Children's Healthcare of Atlanta (CHOA) Foundation Events.

CHOA is a pediatric healthcare system hosting major fundraising events including:
- Hope and Will Ball (January gala)
- Scrubs Party (March benefit)
- CHD Awareness Week (February)
- Community events and campaigns

Site structure: Likely uses Wordpress events or custom CMS.
"""
...
BASE_URL = "https://www.choa.org"
EVENTS_URL = f"{BASE_URL}/donors-and-volunteers/foundation-events"

VENUE_DATA = {
    "name": "Children's Healthcare of Atlanta",
    "slug": "choa-community-events",
    ...
    "website": BASE_URL,
    "vibes": ["pediatric", "childrens-health", "family", "fundraiser"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["ball", "gala", "soiree", "benefit dinner"]):
        return "fundraiser"
    if any(word in text for word in ["scrubs party", "cocktail party", "party"]):
        return "fundraiser"
    if any(word in text for word in ["walk", "run", "5k", "10k", "marathon", "race"]):
        return "fitness"
    if any(word in text for word in ["awareness", "campaign", "advocacy"]):
        return "community"
    if any(word in text for word in ["golf", "tournament"]):
        return "sports"
    if any(word in text for word in ["auction", "silent auction"]):
        return "fundraiser"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["pediatric", "childrens-health"]

    if any(word in text for word in ["gala", "ball"]):
        tags.append("gala")
    if any(word in text for word in ["fundraiser", "benefit", "donation", "support"]):
        tags.append("fundraiser")
    if any(word in text for word in ["chd", "congenital heart", "heart defect"]):
        tags.append("congenital-heart-disease")
    if any(word in text for word in ["awareness", "advocacy"]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl CHOA Foundation events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching CHOA events: {EVENTS_URL}")
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Children's Healthcare of Atlanta", start_date
                        )
                        ...
                        if link_elem:
                            ...
                            if href:
                                ...
                                # Fetch full description if we have a detail URL
        ...
        logger.info(
            f"CHOA crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching CHOA events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl CHOA: {e}")
        raise
