...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Atlanta Freethought Society events.

    Tries /events first, then /calendar, then homepage.
    """
    source_id = source["id"]
    events_found = 0
    events_new = 0
    ...
    try:
        ...
        body_text = ""
        selected_url = EVENTS_URL
        for url in [EVENTS_URL, f"{BASE_URL}/calendar", BASE_URL]:
            try:
                logger.info(f"Fetching Atlanta Freethought Society events: {url}")
        ...
        if not body_text:
            logger.error("Atlanta Freethought Society: could not load any page")
            return 0, 0, 0
