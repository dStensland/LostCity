"""
Crawler for C4 Atlanta (c4atlanta.org/events).

C4 Atlanta is a coalition for community organizing and social justice.
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["meeting", "organizing", "action", "rally", "march", "protest", "campaign"]):
        return "community"
    if any(word in text for word in ["workshop", "class", "training", "education", "teach-in"]):
        return "education"
    if any(word in text for word in ["fundraiser", "gala", "benefit"]):
        return "community"
    if any(word in text for word in ["social", "gathering", "mixer"]):
        return "community"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["activism", "organizing", "community"]

    if any(word in text for word in ["volunteer", "action"]):
        tags.append("volunteer")
    if any(word in text for word in ["justice", "equity", "rights"]):
        tags.append("social-justice")
    if any(word in text for word in ["housing", "tenant"]):
        tags.append("housing")
    if any(word in text for word in ["labor", "worker", "union"]):
        tags.append("labor")
    if any(word in text for word in ["education", "training", "learn", "workshop"]):
        tags.append("education")
    if any(word in text for word in ["climate", "environment"]):
        tags.append("environmental")
    if any(word in text for word in ["health", "healthcare"]):
        tags.append("health")
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    # Default to True for community organizing events
    return True


def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl C4 Atlanta events using Playwright.
    ...
    try:
        with sync_playwright() as p:
            ...
            # Get venue ID for C4 Atlanta
            venue_id = get_or_create_venue(C4_ATLANTA_HQ)

            logger.info(f"Fetching C4 Atlanta events: {EVENTS_URL}")
            ...
            if not events:
                ...
                while i < len(lines):
                    ...
                    if len(line) < 10 or line.lower() in ["events", "upcoming", "past events", "calendar"]:
                        i += 1
                        continue
        ...
        logger.info(
            f"C4 Atlanta crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching C4 Atlanta: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl C4 Atlanta: {e}")
        raise
