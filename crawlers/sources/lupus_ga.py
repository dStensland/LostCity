"""
Crawler for Lupus Foundation of America Georgia Chapter (lupus.org/georgia).

The Lupus Foundation of America Georgia Chapter provides support, education,
and awareness programs for individuals living with lupus and their families
across Georgia.

Events include:
- Support group meetings (in-person and virtual)
- Walk to End Lupus Now fundraising events
- Educational seminars and webinars about lupus
- Awareness and advocacy campaigns
- Community events and health fairs
- Volunteer opportunities

The organization serves lupus patients, families, and caregivers throughout
Georgia.
...
def categorize_event(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    """
    Categorize Lupus Foundation events based on content.

    Returns: (category, subcategory, tags)
    """
    text = f"{title} {description}".lower()
    tags = ["lupus", "chronic-disease", "community-health"]

    # Support groups
    if any(kw in text for kw in [
        "support group", "peer support", "support meeting",
        "caregiver support", "family support"
    ]):
        tags.extend(["support-group", "mental-health"])
        return "wellness", "support_group", tags

    # Walk to End Lupus Now (fundraising)
    if any(kw in text for kw in [
        "walk to end lupus", "walk", "5k", "fundraising walk"
    ]):
        tags.extend(["fundraising", "awareness", "walk"])
        return "community", "fundraiser", tags

    # Educational seminars and webinars
    if any(kw in text for kw in [
        "seminar", "webinar", "workshop", "education",
        "learn", "presentation", "lecture", "class"
    ]):
    ...
    if any(kw in text for kw in [
        "awareness", "advocacy", "campaign", "lupus awareness",
        "world lupus day", "purple"
    ]):
    ...
    if any(kw in text for kw in [
        "health fair", "health screening", "wellness event",
        "health expo"
    ]):
        tags.append("health-fair")
        return "wellness", "health_fair", tags
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Lupus Foundation of America Georgia events using BeautifulSoup.
    ...
    try:
        ...
        logger.info(
            f"Lupus Foundation Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except requests.RequestException as e:
        logger.error(f"Failed to fetch Lupus Foundation Georgia events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Lupus Foundation Georgia: {e}")
        raise
