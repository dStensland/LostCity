"""
Crawler for La Leche League of Georgia (lllofga.org).

La Leche League of Georgia is a breastfeeding support organization offering:
- In-person support meetings across Georgia
- Virtual support meetings
- Peer-to-peer breastfeeding counseling
- Maternal health education
- Free resources for nursing mothers

Meetings are held at various locations throughout Atlanta and Georgia.

STRATEGY:
- Scrape meeting calendar from website
- Extract both in-person and virtual meetings
- Tag: breastfeeding, maternal-health, support-group, free
- Category: "support_group" or "community"
- Most events are free peer support meetings
"""
...
BASE_URL = "https://www.lllofga.org"
MEETINGS_URL = f"{BASE_URL}"

VENUE_DATA = {
    "name": "La Leche League of Georgia",
    "slug": "la-leche-league-georgia",
    "address": "Atlanta, GA",
    ...
    "website": BASE_URL,
    "vibes": ["breastfeeding", "maternal-health", "support"],
}
...
def parse_time_string(time_str: str) -> Optional[str]:
    ...
    try:
        ...
        # Handle ranges, extract first time
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["breastfeeding", "maternal-health", "support-group"]

    # Support group meetings
    if any(word in text for word in ["support", "meeting", "group"]):
        category = "support_group"
    # Educational events
    elif any(word in text for word in ["workshop", "class", "education", "training"]):
        category = "learning"
    ...
    # Virtual events
    if any(word in text for word in ["virtual", "online", "zoom", "video"]):
        tags.append("virtual")
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'meeting|support|breastfeeding', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    try:
        ...
        if not events or len(events) == 0:
            logger.info("No structured meeting elements found")
            logger.info(f"La Leche League Georgia venue record ensured (ID: {venue_id})")
            return 0, 0, 0

        # Parse each meeting/event
        for event_elem in events:
            try:
                # Extract title
                title_elem = event_elem.select_one("h1, h2, h3, h4, .title, .meeting-title, [class*='title']")
                if title_elem:
                ...
                # Skip non-meeting content
                skip_patterns = ["about", "contact", "resources", "donate"]
                if any(pattern in title.lower() for pattern in skip_patterns):
                    continue

                # Extract date
                date_elem = event_elem.select_one(".date, .meeting-date, [class*='date'], time")
                date_str = None
                if date_elem:
                ...
                # If no date element, search text
                if not date_str:
                ...
                # Extract time
                time_elem = event_elem.select_one(".time, .meeting-time, [class*='time']")
                time_str = None
                if time_elem:
                ...
                # Extract description
                description = None
                desc_elem = event_elem.select_one(".description, .meeting-description, .excerpt, .summary, p")
                if desc_elem:
        ...
        logger.info(
            f"La Leche League Georgia crawl complete: {events_found} found, "
