"""
Crawler for ZERO Prostate Cancer Atlanta (zerocancer.org).

ZERO Prostate Cancer is a national nonprofit dedicated to ending prostate cancer
through awareness, screening programs, and support for patients and families.
The Atlanta chapter hosts free PSA screenings, awareness events, and the CHAMPS
Men's Health Summit.

Key Events:
- Free PSA screening events (September at Northside Hospital)
- CHAMPS Men's Health Summit
- Educational seminars and support groups
- Awareness walks and fundraising events

Relevant for hospital patients and at-risk populations (men over 40, African American men).

STRATEGY:
- Scrape the events page for Atlanta-area events
- Extract screening events, health summits, awareness programs
- Tag with: prostate-cancer, screening, mens-health, free-clinic
- Most screening and awareness events are free
- Category: "community" for screenings/awareness, "learning" for education
"""
...
BASE_URL = "https://zerocancer.org"
EVENTS_URL = f"{BASE_URL}/get-involved/attend-an-event/"

VENUE_DATA = {
    "name": "ZERO Prostate Cancer Atlanta",
    "slug": "zero-prostate-cancer-atlanta",
    "address": "5670 Peachtree Dunwoody Rd NE",
    ...
    "website": BASE_URL,
    "vibes": ["prostate-cancer", "screening", "mens-health", "free-clinic"],
}
...
def parse_time_string(time_str: str) -> Optional[str]:
    ...
    Examples: '6:00 PM', '12:30 PM', '9:00 AM - 1:00 PM'
    """
    try:
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["prostate-cancer", "mens-health"]
    ...
    if any(word in text for word in ["free", "no cost", "complimentary", "no charge"]):
        ...
        # Fundraisers typically not free
        if "fundraiser" in tags:
            is_free = False
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|calendar|screening|summit', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl ZERO Prostate Cancer Atlanta events.
    ...
    try:
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"ZERO Prostate Cancer Atlanta venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                if not any(loc in event_text for loc in ["atlanta", "georgia", "ga", "northside"]):
                ...
                event_record = {
                    ...
                    "price_min": None,
                    "price_max": None,
                    "price_note": "Free screening events" if is_free else None,
                    "is_free": is_free,
        ...
        logger.info(
            f"ZERO Prostate Cancer Atlanta crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching ZERO Prostate Cancer events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl ZERO Prostate Cancer Atlanta: {e}")
        raise
