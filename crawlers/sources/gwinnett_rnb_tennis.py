"""
Crawler for Gwinnett County RNB Tennis classes.

Targets the official public RNB Tennis sessions in Gwinnett's Rec1 catalog and
expands them into recurring youth class instances.
"""
...
VENUE_DATA_BY_LOCATION = {
    "mountain park park": {
        ...
        "website": CATALOG_URL,
        "description": "Gwinnett County recreation center hosting public RNB tennis classes.",
    },
    "rhodes jordan park": {
...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    title = f"RNB Tennis at {place_data['name']}"
    description = (
        f"Public youth tennis class at {place_data['name']} through Gwinnett County Parks & Recreation. "
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not checkout_key:
        logger.error("Gwinnett RNB tennis crawl aborted: missing checkout key")
        return 0, 0, 0

    tabs = _get_tabs(TENANT_SLUG, checkout_key)
    sports_tab_id = next((str(tab["id"]) for tab in tabs if tab.get("label") == TARGET_TAB), None)
    if not sports_tab_id:
        logger.error("Gwinnett RNB tennis crawl aborted: Sports tab missing")
        return 0, 0, 0

    groups = _get_groups_for_tab(TENANT_SLUG, checkout_key, sports_tab_id)
    target_group = next((group for _, group in groups if group.get("name") == TARGET_GROUP), None)
    if not target_group:
        logger.error("Gwinnett RNB tennis crawl aborted: Tennis group missing")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info("Removed %s stale Gwinnett RNB tennis rows", stale_removed)

    logger.info(
        "Gwinnett RNB tennis crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
