...
- 20+ support groups across Georgia
- Educational workshops and webinars
- Awareness campaigns and advocacy events
- Caregiver training and resources
- Community education programs

Service area: Statewide Georgia with headquarters in Atlanta (Druid Hills).
...
# Brain Injury Association of Georgia headquarters
VENUE_DATA = {
...
def categorize_event(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    """
    Categorize BIAGA events based on content.
    ...
    if any(kw in text for kw in [
        "support group", "survivor support", "caregiver support",
        "family support", "peer support", "tbi support"
    ]):
    ...
    # Annual conference
    if any(kw in text for kw in [
        "conference", "annual conference", "summit", "symposium",
    ...
    if any(kw in text for kw in [
        "workshop", "training", "seminar", "class",
        "webinar", "education", "learn", "presentation"
    ]):
    ...
    if any(kw in text for kw in [
        "awareness", "advocacy", "brain injury awareness month",
        "awareness walk", "awareness campaign", "legislative"
    ]):
    ...
    # Caregiver programs
    if any(kw in text for kw in [
        "caregiver", "caregiver training", "family training",
    ...
    # Community events
    if any(kw in text for kw in [
        "community", "social", "gathering", "celebration",
        "fundraiser", "benefit"
    ]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Brain Injury Association of Georgia events calendar.
    ...
    try:
        ...
        logger.info(f"Fetching BIAGA events: {EVENTS_URL}")
        ...
        logger.info(
            f"BIAGA crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except requests.RequestException as e:
        logger.error(f"Failed to fetch BIAGA events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl BIAGA: {e}")
        raise
