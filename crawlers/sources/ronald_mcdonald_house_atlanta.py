"""
Crawler for Ronald McDonald House Charities of Atlanta (ronaldmcdonaldhouseatl.org).

RMHC Atlanta provides housing and support for families with hospitalized children.
Events include:
- Golf tournament (May)
- Gala (October)
- Handbag Hullabaloo
...
BASE_URL = "https://ronaldmcdonaldhouseatl.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Ronald McDonald House Charities of Atlanta",
    "slug": "ronald-mcdonald-house-atlanta",
    ...
    "website": BASE_URL,
    "vibes": ["pediatric", "family-support", "hospital", "fundraiser"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["golf", "tournament"]):
        return "sports"
    if any(word in text for word in ["tennis"]):
        return "sports"
    if any(word in text for word in ["gala", "hullabaloo", "benefit"]):
        return "fundraiser"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Ronald McDonald House Atlanta events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Ronald McDonald House Atlanta events: {EVENTS_URL}")
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Ronald McDonald House Charities of Atlanta", start_date
                        )
        ...
        logger.info(
            f"Ronald McDonald House Atlanta crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Ronald McDonald House Atlanta: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Ronald McDonald House Atlanta: {e}")
        raise
