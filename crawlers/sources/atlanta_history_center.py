        ...
        with sync_playwright() as p:
            ...
            while i < len(lines):
                ...
                if date_info:
                    ...
                    desc_or_raw = event_record.get("description") or event_record.get("raw_text") or ""
                    _, range_end = parse_date_range(desc_or_raw)
                    if range_end:
                        event_record["end_date"] = range_end

                    # --- Exhibit detection ---
                    _title_lower = event_record.get("title", "").lower()
                    _desc_lower = (event_record.get("description") or "").lower()
                    _combined_text = f"{_title_lower} {_desc_lower}"
                    exhibit_keywords = ["exhibit", "exhibition", "on view", "collection", "installation"]
                    if any(kw in _combined_text for kw in exhibit_keywords):
                        event_record["content_kind"] = "exhibit"

                    # --- is_all_day for exhibits ---
                    if event_record.get("content_kind") == "exhibit":
                    ...
                    # --- Enrichment ---
                    enrich_event_record(event_record, source_name="Atlanta History Center")
