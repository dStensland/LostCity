"""
Crawler for Muscular Dystrophy Association Greater Georgia (mda.org).

MDA provides comprehensive care and support for individuals with muscular dystrophy,
ALS, and related neuromuscular diseases.

Events include:
- Night of Hope Gala (March)
- Muscle Walk Atlanta
- MDA Summer Camp
- Support groups
- Educational programs

MDA Care Center at Emory University Hospital.
Highly relevant for hospital portal.
"""
...
BASE_URL = "https://www.mda.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Muscular Dystrophy Association Greater Georgia",
    "slug": "mda-georgia",
    ...
    "website": BASE_URL,
    "vibes": ["muscular-dystrophy", "neuromuscular", "support"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["muscle walk", "walk", "run", "5k"]):
        return "fitness"

    if any(word in text for word in ["gala", "night of hope", "fundraiser", "benefit"]):
        return "community"

    if any(word in text for word in ["camp", "summer camp"]):
        return "kids"

    if any(word in text for word in ["support group", "caregiver support"]):
        return "support_group"

    if any(word in text for word in ["workshop", "seminar", "education"]):
        return "learning"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["muscular-dystrophy", "neuromuscular"]
    ...
    if "gala" in text or "fundrais" in text:
        tags.extend(["gala", "fundraiser"])

    if "walk" in text or "run" in text:
        tags.extend(["outdoor", "activism"])

    if "camp" in text or "kids" in text:
        tags.append("kids")
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    if "support group" in text or "camp" in text:
        return True

    if any(word in text for word in ["gala", "benefit", "ticket", "$"]):
        return False
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl MDA Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching MDA Georgia events: {EVENTS_URL}")
            ...
            for event_elem in events:
                try:
                    ...
                    content_hash = generate_content_hash(title, "MDA Georgia", start_date)
        ...
        logger.info(f"MDA Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated")

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching MDA Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl MDA Georgia: {e}")
        raise
