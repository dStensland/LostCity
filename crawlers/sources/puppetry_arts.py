...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    try:
        with sync_playwright() as p:
            ...
            for month_offset in range(2):
                if month_offset > 0:
                    # Click next month
                    try:
                        next_btn = page.query_selector("text=/March 2026 >|February 2026 >/")
                        if next_btn:
                            next_btn.click()
                ...
                while i < len(lines):
                    ...
                    month_match = re.search(r"<?:?\s*(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})", line)
                    if month_match:
                    ...
                    if line == "PUPPET SHOWS" and current_month and current_day:
                        if i + 2 < len(lines):
                            ...
                            skip_words = ["Collection", "Exhibition", "PERMANENT", "CLOSED"]
                            ...
                            # Build date
                            try:
                                dt = datetime.strptime(f"{current_month} {current_day} {current_year}", "%B %d %Y")
                            ...
                            content_hash = generate_content_hash(show_title, "Center for Puppetry Arts", start_date)
                            ...
                            event_record = {
                                ...
                                "tags": ["puppetry", "family", "museum", "performing-arts"],
                                "price_min": None,
                                "price_max": None,
                                "price_note": None,
                                "is_free": False,
        ...
        logger.info(f"Puppetry Arts crawl complete: {events_found} found, {events_new} new")
