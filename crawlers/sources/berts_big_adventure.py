"""
Crawler for Bert's Big Adventure (bertsbigadventure.org).

Bert's Big Adventure provides magical experiences for children with chronic
and terminal illnesses. Events include:
- Family Reunion Adventures (year-round)
...
BASE_URL = "https://bertsbigadventure.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Bert's Big Adventure",
    "slug": "berts-big-adventure",
    "address": "1819 Peachtree Rd NE, Suite 700",
    ...
    "website": BASE_URL,
    "vibes": ["chronic-illness", "pediatric", "family", "volunteer"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["gala", "benefit", "fundraiser"]):
        return "fundraiser"
    if any(word in text for word in ["reunion", "adventure", "family"]):
        return "community"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["chronic-illness", "pediatric"]
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Bert's Big Adventure events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Bert's Big Adventure events: {EVENTS_URL}")
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Bert's Big Adventure", start_date
                        )
        ...
        logger.info(
            f"Bert's Big Adventure crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Bert's Big Adventure: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Bert's Big Adventure: {e}")
        raise
