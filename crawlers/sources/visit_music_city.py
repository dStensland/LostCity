"""
Crawler for Visit Music City Events (visitmusiccity.com/nashville-events).
Official Nashville tourism board - major events and attractions.
...
BASE_URL = "https://www.visitmusiccity.com"
EVENTS_URL = f"{BASE_URL}/nashville-events"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Visit Music City events using Playwright.
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Visit Music City events: {EVENTS_URL}")
            ...
            # Parse event listings - common selectors for event cards
            cards = page.query_selector_all(
                "article, .event, .event-item, .event-card, .card"
            )

            logger.info(f"Found {len(cards)} potential event cards on Visit Music City")
            ...
            for card in cards:
                try:
                    ...
                    date_el = card.query_selector(
                        "time, .date, .event-date, [itemprop='startDate']"
                    )
                    start_date = None
                    end_date = None
                    start_time = None
                    ...
                    venue_el = card.query_selector(
                        ".venue, .location, .event-venue, [itemprop='location']"
                    )
                    venue_name = (
                        venue_el.inner_text().strip() if venue_el else "Nashville Area"
                    )
                    ...
                    event_record = {
                        ...
                        "end_time": None,
                        "is_all_day": end_date
                        is not None,  # Multi-day events are typically all-day
                        "category": category,
                        "subcategory": None,
                        "tags": ["visit-music-city"],
                        "price_min": None,
                        "price_max": None,
                        "price_note": None,
                        "is_free": False,
        ...
        logger.info(
            f"Visit Music City crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Visit Music City: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Visit Music City: {e}")
        raise
