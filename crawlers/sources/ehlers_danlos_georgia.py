"""
Crawler for Ehlers-Danlos Society Georgia Network (ehlers-danlos.com).

EDS Society Georgia provides support for people with Ehlers-Danlos syndrome:
- Monthly Zoom support groups (1,000+ members)
- Female Health Summit
- Educational webinars
- Community events
- Rare disease advocacy

STRATEGY:
- Scrape events page or Georgia chapter page
- Extract support groups, webinars, and summits
- Tag: ehlers-danlos, connective-tissue, rare-disease, support-group
...
BASE_URL = "https://www.ehlers-danlos.com"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Ehlers-Danlos Society Georgia Network",
    "slug": "ehlers-danlos-georgia",
    "address": "Atlanta, GA",
    "neighborhood": "North Atlanta",
    "city": "Atlanta",
    "state": "GA",
    "zip": "30308",
    "lat": 33.8680,
    ...
    "website": BASE_URL,
    "vibes": ["ehlers-danlos", "connective-tissue", "rare-disease"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["ehlers-danlos", "connective-tissue", "rare-disease"]
    ...
    # Summit/conference
    elif any(word in text for word in ["summit", "conference", "symposium"]):
        category = "learning"
        tags.append("conference")
    # Educational
    elif any(word in text for word in ["webinar", "workshop", "education", "training"]):
        category = "learning"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Ehlers-Danlos Society Georgia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"EDS Georgia venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "Ehlers-Danlos Society Georgia Network", start_date
                )
        ...
        logger.info(
            f"EDS Georgia crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching EDS Georgia events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl EDS Georgia: {e}")
        raise
