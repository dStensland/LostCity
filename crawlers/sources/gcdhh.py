"""
Crawler for Georgia Center for the Deaf and Hard of Hearing (gcdhh.org).

GCDHH provides services and advocacy for deaf and hard-of-hearing Georgians:
- ASL classes (beginner to advanced)
- Community events
- Advocacy programs
...
STRATEGY:
- Scrape events/classes page
- Extract ASL classes, community events, and workshops
- Tag: deaf, hard-of-hearing, accessibility, asl, education
- Category: "learning" for classes, "community" for events
"""
...
BASE_URL = "https://www.gcdhh.org"
EVENTS_URL = f"{BASE_URL}"

VENUE_DATA = {
    "name": "Georgia Center for the Deaf and Hard of Hearing",
    "slug": "georgia-center-deaf-hard-of-hearing",
    "address": "4151 Memorial Dr, Suite 103-B",
    ...
    "website": BASE_URL,
    "vibes": ["deaf", "hard-of-hearing", "accessibility"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["deaf", "hard-of-hearing", "accessibility"]

    # ASL classes
    if any(word in text for word in ["asl", "sign language", "class", "level 1", "level 2", "beginner", "intermediate"]):
        category = "learning"
        tags.extend(["asl", "education"])
    # Workshops
    elif any(word in text for word in ["workshop", "training", "seminar"]):
        category = "learning"
        tags.append("education")
    # Community events
    elif any(word in text for word in ["community", "meeting", "event", "gathering"]):
        category = "community"
    else:
        category = "community"
    ...
    if any(word in text for word in ["free", "no cost"]):
        tags.append("free")
        is_free = True
    elif any(word in text for word in ["$", "fee", "cost", "tuition"]):
        is_free = False
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl GCDHH events and classes."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"GCDHH venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "Georgia Center for the Deaf and Hard of Hearing", start_date
                )
        ...
        logger.info(
            f"GCDHH crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching GCDHH events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl GCDHH: {e}")
        raise
