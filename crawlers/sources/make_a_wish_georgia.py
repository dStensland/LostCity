"""
Crawler for Make-A-Wish Georgia (wish.org/georgia).

Make-A-Wish Georgia grants wishes for children with critical illnesses.
Major events include:
- Walk For Wishes (March, May, October 2026)
- Grant-A-Wish Classic golf tournament
- Chili for Children fundraiser
"""
...
BASE_URL = "https://wish.org/georgia"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Make-A-Wish Georgia",
    "slug": "make-a-wish-georgia",
    "address": "1775 The Exchange SE, Suite 200",
    ...
    "website": BASE_URL,
    "vibes": ["wish-granting", "pediatric", "community", "fundraiser"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["walk", "run", "5k", "10k"]):
        return "fitness"
    if any(word in text for word in ["golf", "tournament"]):
        return "sports"
    if any(word in text for word in ["gala", "benefit", "fundraiser", "chili"]):
        return "fundraiser"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Make-A-Wish Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Make-A-Wish Georgia events: {EVENTS_URL}")
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Make-A-Wish Georgia", start_date
                        )
        ...
        logger.info(
            f"Make-A-Wish Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Make-A-Wish Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Make-A-Wish Georgia: {e}")
        raise
