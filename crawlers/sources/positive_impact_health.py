"""
Crawler for Positive Impact Health Centers (positiveimpacthealthcenters.org).

Positive Impact Health Centers provides HIV/STI testing, PrEP, behavioral health,
and comprehensive care for LGBTQ+ communities in Atlanta.
...
- Pride and community celebration events

Service locations: Lindbergh (main), East Point, Gwinnett
"""
...
BASE_URL = "https://www.positiveimpacthealthcenters.org"
...
VENUE_DATA = {
    ...
    "slug": "positive-impact-health",
    "address": "729 Lambert Dr NE",
...
def categorize_event(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    """
    Categorize Positive Impact events based on content.

    Returns: (category, subcategory, tags)
    """
    text = f"{title} {description}".lower()
    tags = ["community-health", "lgbtq", "free"]

    # Testing events
    if any(kw in text for kw in [
        "testing", "hiv test", "sti test", "std test",
        "screening", "rapid test", "get tested"
    ]):
        tags.extend(["hiv-testing", "health-screening"])
        return "wellness", "health_screening", tags

    # PrEP/HIV prevention
    if any(kw in text for kw in [
        "prep", "prevention", "u=u", "undetectable",
        "hiv prevention", "pep"
    ]):
        tags.extend(["hiv-prevention", "education"])
        return "wellness", "health_program", tags

    # Support groups
    if any(kw in text for kw in [
        "support group", "peer support", "mental health",
    ...
    if any(kw in text for kw in [
        "workshop", "education", "training", "class",
        "seminar", "learn", "information session"
    ]):
    ...
    if any(kw in text for kw in [
        "pride", "social", "community event", "celebration",
        "mixer", "party", "gathering", "happy hour"
    ]):
        tags.extend(["community", "social", "lgbtq"])
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Positive Impact Health Centers events.
    """
    source_id = source["id"]
    events_found = 0
    events_new = 0
    ...
    try:
        ...
        for url in EVENTS_URLS:
            try:
                logger.info(f"Trying Positive Impact events URL: {url}")
        ...
        if not events_page_html:
            logger.warning("Could not find Positive Impact events page at any known URL")
        ...
        logger.info(
            f"Positive Impact crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl Positive Impact: {e}")
        raise
