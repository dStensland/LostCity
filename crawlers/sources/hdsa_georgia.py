"""
Crawler for Huntington's Disease Society of America Georgia Chapter (georgia.hdsa.org).

HDSA Georgia provides support and resources for individuals and families affected by
Huntington's disease, a fatal genetic neurological disorder.

Events include:
- Monthly support groups (typically 2nd Thursday)
- Team Hope Walk (annual fundraiser)
- Educational workshops
- Family conferences
- Advocacy events

Relevant for hospital portal (neurological care, family support, Emory affiliation).
"""
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl HDSA Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching HDSA Georgia events: {EVENTS_URL}")
            ...
            for event_elem in events:
                try:
                    ...
                    content_hash = generate_content_hash(title, "HDSA Georgia", start_date)
        ...
        logger.info(f"HDSA Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated")

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching HDSA Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl HDSA Georgia: {e}")
        raise
