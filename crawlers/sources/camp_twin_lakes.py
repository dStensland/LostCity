"""
Crawler for Camp Twin Lakes (camptwinlakes.org).

Camp Twin Lakes provides camp experiences for children with serious illnesses,
disabilities, and life challenges. Over 70 partner programs throughout the year.

Events include:
- Partner camp sessions (Camp Strong4Life, Camp Oasis, Camp Victory, etc.)
- Fundraising events
- Community programs
"""
...
BASE_URL = "https://camptwinlakes.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Camp Twin Lakes",
    "slug": "camp-twin-lakes",
    "address": "1391 Keencheefoonee Rd",
    ...
    "website": BASE_URL,
    "vibes": ["camp", "disabilities", "chronic-illness", "inclusive", "youth"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["camp", "session", "weekend", "retreat"]):
        return "community"
    if any(word in text for word in ["gala", "fundraiser", "benefit", "auction"]):
        return "fundraiser"
    if any(word in text for word in ["volunteer", "orientation", "training"]):
        return "community"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["camp", "disabilities", "inclusive"]

    if any(word in text for word in ["chronic illness", "medical", "illness"]):
        tags.append("chronic-illness")
    if any(word in text for word in ["cancer", "oncology"]):
        tags.append("cancer")
    if any(word in text for word in ["diabetes"]):
        tags.append("diabetes")
    if any(word in text for word in ["autism", "developmental"]):
        tags.append("autism")
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Camp Twin Lakes events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Camp Twin Lakes events: {EVENTS_URL}")
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Camp Twin Lakes", start_date
                        )
        ...
        logger.info(
            f"Camp Twin Lakes crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Camp Twin Lakes: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Camp Twin Lakes: {e}")
        raise
