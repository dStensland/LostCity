"""
Crawler for Cobb County adult swim lessons.

Targets the official Rec1 adult swim lessons group and expands the county's
published weekday/date-range schedule into recurring adult class instances.
"""
...
TARGET_TAB_ID = "3454"
TARGET_GROUP = "Adult Basics Swimming Lessons"
WEEKS_AHEAD = 10
...
PLACE_DATA = {
    ...
    "website": CATALOG_URL,
    "description": "Cobb County aquatic center hosting public adult swim lessons.",
}
...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    title = f"Adult Basics Swimming Lessons at {PLACE_DATA['name']}"
    description = (
        f"Adult swim lessons at {PLACE_DATA['name']} through Cobb County Parks. "
    ...
    return {
        ...
        "raw_text": (
            f"Adult Basics Swimming Lessons | {features.get('days', '')} | "
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not checkout_key:
        logger.error("Cobb adult swim lessons crawl aborted: missing checkout key")
        return 0, 0, 0

    tabs = _get_tabs(TENANT_SLUG, checkout_key)
    if not any(str(tab.get("id")) == TARGET_TAB_ID for tab in tabs):
        logger.error("Cobb adult swim lessons crawl aborted: target tab missing")
        return 0, 0, 0
    ...
    if not target_group:
        logger.error("Cobb adult swim lessons crawl aborted: target group missing")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info("Removed %s stale Cobb adult swim lesson rows", stale_removed)

    logger.info(
        "Cobb adult swim lessons crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
