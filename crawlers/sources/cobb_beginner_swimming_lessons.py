"""
Crawler for Cobb County beginner swim lessons.

Targets the official Rec1 beginner swim lessons group and expands the county's
...
TARGET_TAB_ID = "3454"
TARGET_GROUP = "Beginner Swimming Lessons"
WEEKS_AHEAD = 10
...
VENUE_DATA = {
    ...
    "website": CATALOG_URL,
    "description": "Cobb County aquatic center hosting beginner swim lessons.",
}
...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    title = f"Beginner Swimming Lessons at {VENUE_DATA['name']}"
    description = (
        f"Beginner swim lessons at {VENUE_DATA['name']} through Cobb County Parks. "
    ...
    return {
        ...
        "raw_text": (
            f"Beginner Swimming Lessons | {features.get('days', '')} | "
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not any(str(tab.get("id")) == TARGET_TAB_ID for tab in tabs):
        logger.error("Cobb beginner swimming lessons crawl aborted: target tab missing")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info(
            "Removed %s stale Cobb beginner swimming lesson rows",
            stale_removed,
        )

    logger.info(
        "Cobb beginner swimming lessons crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
