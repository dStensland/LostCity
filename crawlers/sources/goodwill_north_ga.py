"""
Crawler for Goodwill of North Georgia (ging.org).

Goodwill of North Georgia provides career training, job placement services, and
workforce development programs. They host career training orientations, hiring events,
and digital skills workshops.

Located in East Atlanta, Goodwill is a major workforce development provider serving
the Atlanta metro area.

STRATEGY:
- Scrape the events page at /events or /workforce-development
- Extract career training orientations, hiring events, digital skills workshops
- Tag appropriately: employment, job-training, career, free
- Most events are free for participants
- Category: "learning" for training, "community" for hiring events

Relevant for community workforce development and employment services.
"""
...
BASE_URL = "https://goodwillng.org"
EVENTS_URL = "https://goodwillng.org/events/"
...
VENUE_DATA = {
    ...
    "website": BASE_URL,
    "vibes": ["career", "job-training", "community"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["employment", "job-training"]

    # Career training and orientations
    if any(word in text for word in ["training", "orientation", "program", "class", "certification", "digital skills", "computer training"]):
        category = "learning"
        tags.extend(["career", "training"])

    # Hiring events and job fairs
    elif any(word in text for word in ["hiring", "job fair", "career fair", "employer event", "recruitment"]):
        category = "community"
        tags.extend(["job-fair", "hiring", "career"])

    # Workshop events
    elif any(word in text for word in ["workshop", "seminar", "info session", "information session"]):
        category = "learning"
        tags.extend(["workshop", "career"])
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|calendar|training|workshop', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    try:
        ...
        urls_to_try = [
            EVENTS_URL,
            f"{BASE_URL}/workforce-development",
            f"{BASE_URL}/calendar",
            f"{BASE_URL}/career-services",
            BASE_URL,
        ]
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"Goodwill of North Georgia venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                skip_patterns = ["hours", "schedule", "contact us", "about", "services", "locations", "donate"]
                ...
                content_hash = generate_content_hash(
                    title, "Goodwill of North Georgia", start_date
                )
                ...
                event_record = {
                    ...
                    "price_min": None,
                    "price_max": None,
                    "price_note": "Free career services",
                    "is_free": is_free,
        ...
        logger.info(
            f"Goodwill of North Georgia crawl complete: {events_found} found, "
