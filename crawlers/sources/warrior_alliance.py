"""
Crawler for The Warrior Alliance (thewarrioralliance.org).

The Warrior Alliance is a veteran nonprofit based in The Battery at Truist Park,
providing fitness-based programs, community events, and resource expos for veterans
and their families. Major events include:
- Veteran Benefits & Resource Expos (2,500+ attendees)
- Fitness training programs
- Veteran Impact Awards & Gala
- Ruck marches and community challenges

Located at The Battery Atlanta (Cumberland area), next to Truist Park.

STRATEGY:
- Scrape events page at /events/
- Extract benefit expos, galas, fitness programs, community events
- Tag: veterans, fitness, military, community
- Category: "community" for expos/galas, "fitness" for training, "learning" for workshops
- Most events are free for veterans and families

Site uses WordPress with Elementor - try BeautifulSoup first, fallback to Playwright.
"""
...
BASE_URL = "https://thewarrioralliance.org"
EVENTS_URL = f"{BASE_URL}/events/"

VENUE_DATA = {
    "name": "The Warrior Alliance",
    "slug": "warrior-alliance",
    "address": "800 Battery Ave SE, Suite 100",
    "neighborhood": "The Battery / Cumberland",
    "city": "Atlanta",
    "state": "GA",
    "zip": "30339",
    "lat": 33.8907,
    ...
    "website": BASE_URL,
    "vibes": ["veterans", "fitness", "community"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    if any(word in text for word in ["fitness", "workout", "training", "ruck", "pt", "physical training", "crossfit", "yoga"]):
        category = "fitness"
        tags.extend(["fitness", "training"])

    # Benefit expos and resource fairs
    elif any(word in text for word in ["expo", "resource fair", "benefits", "job fair", "career fair"]):
        category = "community"
        tags.extend(["resources", "benefits", "family-friendly"])

    # Galas and fundraisers
    elif any(word in text for word in ["gala", "awards", "fundraiser", "benefit dinner", "charity"]):
        category = "community"
        tags.extend(["fundraiser", "charity", "awards"])

    # Workshops and educational events
    elif any(word in text for word in ["workshop", "seminar", "training", "class", "education", "learn"]):
        category = "learning"
        tags.extend(["workshop", "education"])

    # Community events
    elif any(word in text for word in ["community", "social", "networking", "mixer", "meetup"]):
        category = "community"
    ...
    # Check if free (most Warrior Alliance events are free for veterans)
    is_free = True
    if any(word in text for word in ["free", "no cost", "no charge", "complimentary", "no fee"]):
        ...
        if "free" in text or "no cost" in text or "free for veterans" in text:
            is_free = True
            tags.append("free")
        else:
            is_free = False
    ...
    if any(word in text for word in ["family", "families", "kids", "children", "youth"]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl The Warrior Alliance events calendar.
    ...
    try:
        ...
        event_selectors = [
            ...
            ".elementor-widget-container",  # Elementor widgets
        ]
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"The Warrior Alliance venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "The Warrior Alliance", start_date
                )
        ...
        logger.info(
            f"The Warrior Alliance crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching The Warrior Alliance events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl The Warrior Alliance: {e}")
        raise
