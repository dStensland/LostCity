"""
Crawler for SisterLove (sisterlove.org).

SisterLove is a reproductive justice organization providing HIV testing,
reproductive health services, and advocacy for women and communities of color.

Programs include:
- Healthy Love Bus (mobile HIV testing)
- Reproductive justice education
- Community health workshops
- HIV prevention and testing events
- Sexual health education
- Advocacy and policy events

Service area: Metro Atlanta with focus on communities of color
"""
...
BASE_URL = "https://www.sisterlove.org"
...
def categorize_event(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    """
    Categorize SisterLove events based on content.

    Returns: (category, subcategory, tags)
    """
    text = f"{title} {description}".lower()
    tags = ["reproductive-health", "community-health", "free"]

    # HIV testing events
    if any(kw in text for kw in [
        "hiv test", "testing", "healthy love bus",
        "mobile testing", "get tested", "screening"
    ]):
    ...
    if any(kw in text for kw in [
        "reproductive", "birth control", "contraception",
        "pregnancy", "maternal health", "sexual health"
    ]):
        tags.extend(["reproductive-justice", "sexual-health"])
        return "wellness", "health_program", tags

    # Educational workshops
    if any(kw in text for kw in [
        "workshop", "education", "training", "class",
        "seminar", "learn", "session"
    ]):
    ...
    # Advocacy and policy
    if any(kw in text for kw in [
        "advocacy", "policy", "rights", "justice",
        "rally", "march", "action"
    ]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    try:
        ...
        for url in EVENTS_URLS:
            try:
                logger.info(f"Trying SisterLove events URL: {url}")
        ...
        if not events_page_html:
            logger.warning("Could not find SisterLove events page at any known URL")
        ...
        logger.info(
            f"SisterLove crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl SisterLove: {e}")
        raise
