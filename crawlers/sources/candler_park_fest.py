"""
Crawler for Candler Park Fall Fest (fallfest.candlerpark.org).
Annual neighborhood festival in Candler Park, Atlanta.
"""
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Candler Park events using requests + BeautifulSoup."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        while i < len(lines):
            ...
            if date_match:
                ...
                content_hash = generate_content_hash(title, "Candler Park", start_date)
                ...
                event_record = {
                    ...
                    "category": "community",
                    "subcategory": None,
                    "tags": ["candler-park", "fall-fest", "music", "family", "neighborhood"],
                    "price_min": None,
                    "price_max": None,
                    "price_note": None,
                    "is_free": False,
        ...
        logger.info(
            f"Candler Park crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl Candler Park: {e}")
        raise
