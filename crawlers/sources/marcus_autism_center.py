"""
Crawler for Marcus Autism Center (marcus.org).

Marcus Autism Center is one of the nation's largest facilities dedicated to the diagnosis
and treatment of children with autism and related developmental disorders. Part of
Children's Healthcare of Atlanta and Emory University School of Medicine.

Events include:
- Grand Rounds and educational seminars for professionals
- Support groups for parents and families
- Social skills groups for children and teens
- Summer camps and recreational programs
- Workshops on autism interventions and therapies

Highly relevant for hospital portal (Emory affiliation, family resources).
"""
...
BASE_URL = "https://www.marcus.org"
EVENTS_URL = f"{BASE_URL}/events-and-workshops"

VENUE_DATA = {
    "name": "Marcus Autism Center",
    "slug": "marcus-autism-center",
    "address": "1920 Briarcliff Rd NE",
    ...
    "website": BASE_URL,
    "vibes": ["autism", "developmental-disabilities", "family"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["support group", "parent support", "family support"]):
        return "support_group"

    if any(word in text for word in ["workshop", "seminar", "training", "lecture", "grand rounds"]):
        return "learning"

    if any(word in text for word in ["social skills", "social group", "teen group"]):
        return "community"

    if any(word in text for word in ["camp", "summer", "recreation"]):
        return "kids"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["autism", "developmental-disabilities"]

    if "family" in text or "parent" in text:
        tags.append("family-friendly")

    if "child" in text or "kids" in text or "teen" in text:
        tags.append("kids")
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    if "support group" in text or "grand rounds" in text:
        return True

    if any(word in text for word in ["$", "registration fee", "tuition"]):
        return False
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Marcus Autism Center events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Marcus Autism Center events: {EVENTS_URL}")
            ...
            for event_elem in events:
                try:
                    ...
                    content_hash = generate_content_hash(title, "Marcus Autism Center", start_date)
        ...
        logger.info(f"Marcus Autism Center crawl complete: {events_found} found, {events_new} new, {events_updated} updated")

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Marcus Autism Center: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Marcus Autism Center: {e}")
        raise
