...
Events include:
- Weekly wellness classes (yoga, meditation, fitness)
...
Service area: Metro Atlanta with headquarters in Pittsburgh neighborhood.
...
# Center for Black Women's Wellness headquarters
VENUE_DATA = {
...
def categorize_event(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    """
    Categorize CBWW events based on content.
    ...
    # Wellness classes (yoga, meditation, fitness)
    if any(kw in text for kw in [
        "yoga", "meditation", "fitness", "exercise",
        "pilates", "zumba", "dance", "movement class",
        "wellness class", "mindfulness"
    ]):
    ...
    if any(kw in text for kw in [
        "workshop", "health education", "seminar", "class",
        "training", "webinar", "learn", "presentation"
    ]):
    ...
    # Annual conference
    if any(kw in text for kw in [
        "conference", "summit", "symposium", "annual conference",
        "wellness conference"
    ]):
    ...
    # Support groups and circles
    if any(kw in text for kw in [
        "support group", "support circle", "healing circle",
        "women's circle", "sister circle", "peer support"
    ]):
    ...
    if any(kw in text for kw in [
        "health screening", "health fair", "community health",
        "nutrition", "chronic disease", "diabetes", "heart health"
    ]):
    ...
    if any(kw in text for kw in [
        "advocacy", "awareness", "health equity", "policy",
        "awareness month", "awareness campaign", "legislative"
    ]):
    ...
    if any(kw in text for kw in [
        ...
        "mixer", "networking", "community event"
    ]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Center for Black Women's Wellness events calendar.
    ...
    try:
        ...
        logger.info(f"Fetching CBWW events: {EVENTS_URL}")
        ...
        logger.info(
            f"CBWW crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except requests.RequestException as e:
        logger.error(f"Failed to fetch CBWW events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl CBWW: {e}")
        raise
