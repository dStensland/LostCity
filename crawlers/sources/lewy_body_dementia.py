"""
Crawler for Lewy Body Dementia Association (lbda.org).

LBDA provides education and support for Lewy body dementia patients and caregivers:
...
National HQ near Atlanta in Lilburn, GA.

STRATEGY:
- Scrape events/webinars page
- Extract monthly webinars and support groups
- Tag: lewy-body-dementia, dementia, neurological, caregiver, support-group
- Category: "learning" for webinars, "support_group" for groups
...
BASE_URL = "https://www.lbda.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Lewy Body Dementia Association",
    "slug": "lewy-body-dementia-association",
    "address": "912 Killian Hill Rd SW, Suite 202C",
    ...
    "website": BASE_URL,
    "vibes": ["dementia", "neurological", "caregiver"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["lewy-body-dementia", "dementia", "neurological"]

    # Support groups
    if any(word in text for word in ["support", "group", "caregiver group"]):
    ...
    # Virtual
    if any(word in text for word in ["webinar", "virtual", "online", "zoom"]):
        tags.append("virtual")
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Lewy Body Dementia Association events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"LBDA venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "Lewy Body Dementia Association", start_date
                )
                ...
                event_record = {
                    ...
                    "price_min": None,
                    "price_max": None,
                    "price_note": "Free webinars and support groups",
                    "is_free": is_free,
        ...
        logger.info(
            f"LBDA crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching LBDA events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl LBDA: {e}")
        raise
