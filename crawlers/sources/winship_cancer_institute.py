"""
Crawler for Winship Cancer Institute of Emory University (winshipcancer.emory.edu).

Winship is Georgia's only National Cancer Institute-designated Comprehensive Cancer Center.
Part of Emory University and Emory Healthcare system.

Events include:
- Learn and Live educational events for patients and families
- Support groups (breast cancer, lung cancer, survivors, etc.)
- Wellness programs (yoga, meditation, nutrition workshops)
- Clinical trial information sessions
- Community outreach and screenings

Core Emory portal content - highly relevant for patients and families.
"""
...
BASE_URL = "https://winshipcancer.emory.edu"
EVENTS_URL = f"{BASE_URL}/support-and-survivorship/winship-support-programs/learn-and-live"
...
VENUE_DATA = {
    ...
    "website": BASE_URL,
    "vibes": ["cancer", "clinical-trials", "research"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["support group", "survivor", "caregiver support"]):
        return "support_group"

    if any(word in text for word in ["yoga", "meditation", "wellness", "exercise", "fitness"]):
        return "wellness"

    if any(word in text for word in ["nutrition", "cooking", "healthy eating"]):
        return "food"

    if any(word in text for word in ["workshop", "seminar", "learn and live", "education", "lecture"]):
        return "learning"

    if any(word in text for word in ["screening", "clinical trial"]):
        return "wellness"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    if any(word in text for word in ["workshop", "education", "learn"]):
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    # Most Learn and Live events are free
    if "learn and live" in text or "support group" in text:
        return True

    if any(word in text for word in ["$", "registration fee", "ticket"]):
        return False
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Winship Cancer Institute events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Winship Cancer Institute events: {EVENTS_URL}")
            ...
            for event_elem in events:
                try:
                    ...
                    content_hash = generate_content_hash(title, "Winship Cancer Institute", start_date)
        ...
        logger.info(f"Winship Cancer Institute crawl complete: {events_found} found, {events_new} new, {events_updated} updated")

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Winship Cancer Institute: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Winship Cancer Institute: {e}")
        raise
