...
def parse_time(time_text: str) -> Optional[str]:
    """Parse time from '9:00 PM' format."""
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Havana Club events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Havana Club: {EVENTS_URL}")
                        ...
                        if 0 <= idx < len(lines):
                            ...
                            if not title and len(check_line) > 5:
                                if not re.match(r"\d{1,2}[:/]", check_line):
                                    if not re.match(
                                        r"(free|tickets|register|\$|more info|rsvp|buy|vip|table)",
                                        check_line.lower(),
                                    ):
                                        title = check_line
                                        break
