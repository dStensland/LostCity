"""
Crawler for WorkSource Atlanta (worksourceatlanta.org).

WorkSource Atlanta provides career services, job training, and workforce development
programs for job seekers in Atlanta. They host job fairs, career workshops, training
info sessions, and youth employment programs.

Located in Lakewood, WorkSource Atlanta is a critical resource for employment and
workforce development in South Atlanta.

STRATEGY:
- Scrape the events/programs page
- Extract job fairs, career workshops, training sessions, youth programs
- Tag appropriately: employment, job-training, career, workforce, free
- Most events are free for job seekers
- Category: "community" for job fairs, "learning" for workshops/training

Relevant for community workforce development initiatives.
"""
...
BASE_URL = "https://www.worksourceatlanta.org"
EVENTS_URL = f"{BASE_URL}/events"  # Will try multiple patterns

VENUE_DATA = {
    "name": "WorkSource Atlanta",
    "slug": "worksource-atlanta",
    "address": "818 Pollard Blvd SW",
    ...
    "website": BASE_URL,
    "vibes": ["career", "job-training", "workforce"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    # Job fair events
    if any(word in text for word in ["job fair", "career fair", "hiring event", "employer event", "virtual job fair"]):
        category = "community"
        tags.extend(["job-fair", "career", "hiring"])

    # Career workshops and training
    elif any(word in text for word in ["workshop", "training", "class", "seminar", "certification", "orientation", "info session"]):
        category = "learning"
        tags.extend(["job-training", "career", "workshop"])

    # Youth employment programs
    elif any(word in text for word in ["youth", "teen", "student", "young adult", "summer jobs"]):
        category = "learning"
        tags.extend(["youth", "career", "job-training"])

    # Resume and interview help
    elif any(word in text for word in ["resume", "interview", "career counseling", "job search"]):
        category = "learning"
        tags.extend(["career", "job-search"])

    # Default to community
    else:
        category = "community"

    # Most WorkSource events are free
    is_free = True
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|calendar|workshop|job fair', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl WorkSource Atlanta events.
    ...
    try:
        ...
        # If simple request didn't work, use Playwright on main events URL
        if not soup:
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"WorkSource Atlanta venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "WorkSource Atlanta", start_date
                )
                ...
                event_record = {
                    ...
                    "price_min": None,
                    "price_max": None,
                    "price_note": "Free for job seekers",
                    "is_free": is_free,
        ...
        logger.info(
            f"WorkSource Atlanta crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching WorkSource Atlanta events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl WorkSource Atlanta: {e}")
        raise
