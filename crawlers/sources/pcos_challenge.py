"""
Crawler for PCOS Challenge National Association (pcoschallenge.org).

PCOS Challenge is the leading patient advocacy organization for women with
Polycystic Ovary Syndrome, headquartered in Atlanta (Virginia-Highland).

Key events and programs:
- PCOS Awareness Symposium (annual flagship event)
- PCOS Advocacy Day (Capitol Hill advocacy)
- Community health screenings
- Educational webinars and workshops
- Support group meetings (local and virtual)
- Walk for PCOS awareness

STRATEGY:
- Scrape events page for symposiums, awareness events, advocacy days
- Tag: pcos, womens-health, symposium, advocacy, awareness
- Mix of free educational events and ticketed symposiums
- Category: "learning" for symposiums/webinars, "community" for walks/support groups
"""
...
BASE_URL = "https://pcoschallenge.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "PCOS Challenge National Association",
    "slug": "pcos-challenge",
    "address": "931 Monroe Dr NE, Suite 102",
    ...
    "website": BASE_URL,
    "vibes": ["pcos", "womens-health", "awareness"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["pcos", "womens-health"]

    # PCOS Awareness Symposium (flagship event)
    if "symposium" in text or "conference" in text:
        category = "learning"
        tags.extend(["symposium", "conference", "education"])
        is_free = False
    ...
    # Walks and awareness events
        ...
        category = "fitness"
        tags.extend(["walk", "outdoor", "awareness"])
        is_free = False
...
def try_simple_requests(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|symposium|advocacy', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl PCOS Challenge National Association events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        logger.info(f"Fetching PCOS Challenge events: {EVENTS_URL}")
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(title, "PCOS Challenge National Association", start_date)
        ...
        logger.info(
            f"PCOS Challenge crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching PCOS Challenge: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl PCOS Challenge: {e}")
        raise
