...
Generates recurring Tuesday trivia and Wednesday comedy events.
"""
...
WEEKLY_SCHEDULE = [
    {
        "day": 1,  # Tuesday
        "title": "Trivia at Argosy",
        "description": (
            "Tuesday trivia night at Argosy in East Atlanta Village. "
            "Pub trivia with craft cocktails and a great patio in EAV."
        ),
        "start_time": "20:00",
        "category": "nightlife",
        "subcategory": "nightlife.trivia",
        "tags": ["trivia", "nightlife", "weekly", "east-atlanta", "craft-cocktails"],
    },
    {
        "day": 2,  # Wednesday
        "title": "Squids N Giggles Comedy at Argosy",
        "description": (
            "Wednesday free comedy night at Argosy in East Atlanta Village. "
            "Squids N Giggles brings local and touring comedians to EAV."
        ),
        "start_time": "20:00",
        "category": "comedy",
        "subcategory": "comedy.standup",
        "tags": ["comedy", "stand-up", "weekly", "east-atlanta", "free"],
    },
]
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Ensure Argosy venue and generate recurring trivia events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    for template in WEEKLY_SCHEDULE:
        ...
        for week in range(WEEKS_AHEAD):
            ...
            event_record = {
                ...
                "ticket_url": None,
                "image_url": None,
                "raw_text": f"{template['title']} at Argosy - {start_date}",
    ...
    logger.info(
        f"Argosy crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
    )
