"""
Crawler for Camp Sunshine Georgia (mycampsunshine.com).

Camp Sunshine Georgia provides camp experiences for children with cancer
and their families. Programs include:
- Summer camp sessions (ages 7-18)
- Family Camp Weekends (quarterly)
- Sunshine Spectacular fundraiser

Uses Camp Twin Lakes Rutledge campus for programs.
"""
...
BASE_URL = "https://www.mycampsunshine.com"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Camp Sunshine Georgia",
    "slug": "camp-sunshine-georgia",
    ...
    "website": BASE_URL,
    "vibes": ["cancer", "pediatric", "camp", "family-support"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["spectacular", "gala", "fundraiser", "benefit"]):
        return "fundraiser"
    if any(word in text for word in ["camp", "family camp", "weekend"]):
        return "community"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["cancer", "pediatric", "camp"]

    if any(word in text for word in ["family", "families"]):
        tags.append("family")
    if any(word in text for word in ["fundraiser", "gala", "spectacular"]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Camp Sunshine Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Camp Sunshine events: {EVENTS_URL}")
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Camp Sunshine Georgia", start_date
                        )
        ...
        logger.info(
            f"Camp Sunshine crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Camp Sunshine: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Camp Sunshine: {e}")
        raise
