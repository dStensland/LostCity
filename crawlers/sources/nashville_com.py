"""
Crawler for Nashville.com Events (nashville.com/calendar-of-events).
Nashville's official tourism site - comprehensive events calendar.
...
BASE_URL = "https://www.nashville.com"
...
def parse_date(date_text: str) -> tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        # "Jan 15" or "January 15" (assume current or next year)
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Nashville.com events using Playwright.
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Nashville.com events: {EVENTS_URL}")
            ...
            # Parse event listings - common selectors
            cards = page.query_selector_all("article, .event, .event-item, .event-card, .card, .post")

            logger.info(f"Found {len(cards)} potential event cards on Nashville.com")
            ...
            for card in cards:
                try:
                    ...
                    # Title
                    title_el = card.query_selector("h1, h2, h3, h4, .title, .event-title, .entry-title")
                    ...
                    venue_name = venue_el.inner_text().strip() if venue_el else "Nashville Area"
                    ...
                    event_record = {
                        ...
                        "end_date": end_date,
                        "end_time": None,
                        "is_all_day": end_date is not None,  # Multi-day events are typically all-day
                        "category": category,
                        "subcategory": None,
                        "tags": ["nashville-com"],
                        "price_min": None,
        ...
        logger.info(
            f"Nashville.com crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Nashville.com: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Nashville.com: {e}")
        raise
