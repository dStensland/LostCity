"""
Crawler for Myasthenia Gravis Georgia (mg-georgia.org).

MG Georgia provides support and education for myasthenia gravis patients:
- Monthly support group (1st Monday 6:30-8pm, hybrid in-person/Zoom)
- Educational events
- Community resources
- Awareness campaigns
...
STRATEGY:
- Scrape events/support group page
- Extract monthly meetings and educational events
- Tag: myasthenia-gravis, neurological, support-group, free
...
BASE_URL = "https://mg-georgia.org"
EVENTS_URL = f"{BASE_URL}"

VENUE_DATA = {
    "name": "Myasthenia Gravis Georgia",
    "slug": "mg-georgia",
    "address": "5285 Peachtree Industrial Blvd",
    ...
    "website": BASE_URL,
    "vibes": ["myasthenia-gravis", "neurological", "support"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["myasthenia-gravis", "neurological"]
    ...
    # Educational
    elif any(word in text for word in ["workshop", "seminar", "education", "training"]):
        category = "learning"
    ...
    # Virtual/hybrid
    if any(word in text for word in ["virtual", "zoom", "online", "hybrid"]):
        tags.append("virtual")
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl MG Georgia support groups and events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"MG Georgia venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "Myasthenia Gravis Georgia", start_date
                )
        ...
        logger.info(
            f"MG Georgia crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching MG Georgia events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl MG Georgia: {e}")
        raise
