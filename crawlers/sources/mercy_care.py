"""
Crawler for Mercy Care Atlanta (mercyatlanta.org).

Mercy Care provides free healthcare to uninsured, working-poor adults in
Metro Atlanta. Operating since 1987, it's one of the largest free clinics
in Georgia.
...
Service area: Metro Atlanta (Fulton, DeKalb, Cobb, Clayton, Gwinnett counties).
...
BASE_URL = "https://mercyatlanta.org"
EVENTS_URL = f"{BASE_URL}/events/"

VENUE_DATA = {
    "name": "Mercy Care Atlanta",
    "slug": "mercy-care-atlanta",
    "address": "305 Jesse Hill Jr Dr SE",
...
def categorize_event(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    """
    Categorize Mercy Care Atlanta events based on content.

    Returns: (category, subcategory, tags)
    """
    text = f"{title} {description}".lower()
    tags = ["community-health", "free-clinic", "health-equity", "community"]

    # Fundraising events
    if any(kw in text for kw in [
        "fall festival", "gala", "fundraiser", "benefit",
        "auction", "donor", "donation"
    ]):
        tags.extend(["fundraiser", "benefit", "family-friendly"])
    ...
    if any(kw in text for kw in [
        "health fair", "screening", "wellness", "clinic",
        "health event", "medical"
    ]):
    ...
    # Community events
    if any(kw in text for kw in [
        "community", "celebration", "appreciation",
        "gathering", "social"
    ]):
        tags.extend(["community-event"])
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Mercy Care Atlanta events using BeautifulSoup.
    ...
    try:
        ...
        logger.info(f"Fetching Mercy Care Atlanta events: {EVENTS_URL}")
        ...
        if not event_containers:
            ...
            event_containers = soup.find_all(["div", "article"], class_=re.compile(r"item|card"))
        ...
        for container in event_containers:
            try:
                ...
                title_elem = container.find(["h3", "h2", "h1"], class_=re.compile(r"title|event|heading"))
                if not title_elem:
        ...
        logger.info(
            f"Mercy Care Atlanta crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except requests.RequestException as e:
        logger.error(f"Failed to fetch Mercy Care Atlanta events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Mercy Care Atlanta: {e}")
        raise
