"""
Crawler for Postpartum Support International Georgia Chapter (psichapters.com/ga/).

PSI Georgia provides critical mental health support for new mothers:
- Postpartum Moms Group Atlanta (at Northside Hospital & Atlanta Medical Center)
- Peer mentor program
- PEACE for Moms program (Emory Healthcare)
- Virtual and in-person support groups
- Resources for postpartum depression, anxiety, and mood disorders

STRATEGY:
- Scrape chapter events/groups page
- Extract support group meetings and educational events
- Tag: postpartum, maternal-mental-health, support-group, free
- Category: "support_group" or "learning"
...
BASE_URL = "https://psichapters.com/ga"
GROUPS_URL = f"{BASE_URL}"
...
VENUE_DATA = {
    ...
    "website": BASE_URL,
    "vibes": ["postpartum", "maternal-mental-health", "support"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["postpartum", "maternal-mental-health"]

    # Support groups
    if any(word in text for word in ["support", "group", "meeting", "peer"]):
        category = "support_group"
        tags.append("support-group")
    # Educational/training
    elif any(word in text for word in ["workshop", "training", "class", "education"]):
        category = "learning"
    ...
    if any(word in text for word in ["virtual", "online", "zoom"]):
        tags.append("virtual")
    if any(word in text for word in ["in-person", "in person"]):
        tags.append("in-person")

    # Free (most PSI groups are free)
    tags.append("free")
    is_free = True
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl PSI Georgia support groups and events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        # Look for group/event containers
        event_selectors = [
            ".group",
            ".event",
            "[class*='group']",
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"PSI Georgia venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                if link_elem:
                    ...
                    if href:
                        if href.startswith("http"):
                            ...
                            event_url = "https://psichapters.com" + href

                category, tags, is_free = determine_category_and_tags(title, description or "")

                content_hash = generate_content_hash(
                    title, "Postpartum Support International Georgia", start_date
                )
                ...
                event_record = {
                    ...
                    "price_min": None,
                    "price_max": None,
                    "price_note": "Free support groups",
                    "is_free": is_free,
        ...
        logger.info(
            f"PSI Georgia crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching PSI Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl PSI Georgia: {e}")
        raise
