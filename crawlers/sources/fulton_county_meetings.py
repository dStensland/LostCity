"""
Crawler for Fulton County Government Meetings via the Legistar REST API.

Source API: https://webapi.legistar.com/v1/fulton/events
Calendar URL: https://fulton.legistar.com/Calendar.aspx
...
Volume: approximately 2 Board of Commissioners meetings per month. Fulton
tends to post meetings on shorter notice than DeKalb — the 90-day lookahead
will often return fewer than the theoretical maximum.
"""
...
API_BASE = "https://webapi.legistar.com/v1/fulton"
CALENDAR_URL = "https://fulton.legistar.com/Calendar.aspx"
LOOKAHEAD_DAYS = 90

# Default meeting venue: Fulton County Government Center Assembly Hall
DEFAULT_VENUE_DATA = {
    "name": "Fulton County Government Center Assembly Hall",
    "slug": "fulton-county-government-center-assembly-hall",
...
def _series_hint_for_body(body_name: str) -> dict:
    ...
    return {
        ...
        "series_title": f"Fulton County {body_name}",
        "frequency": "monthly",
    }


def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Fulton County government meetings via the Legistar REST API."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        try:
            ...
            logger.error("Failed to fetch Fulton Legistar events: %s", exc)
            raise

        logger.info("Legistar returned %d events for Fulton", len(raw_events))

        if not raw_events:
            # Fulton posts meetings on shorter notice — an empty window is normal.
            logger.info(
                "No Fulton meetings scheduled in the next %d days", LOOKAHEAD_DAYS
            )
            return 0, 0, 0
        ...
        for event in raw_events:
            try:
                ...
                # Description
                desc_parts = [
                    f"{body_name} meeting of Fulton County Government.",
                ]
                if agenda_status:
                ...
                logger.warning(
                    "Error processing Fulton event %s: %s",
                    event.get("EventId"),
                    exc,
                )
                continue
        ...
        logger.info(
            "Fulton County crawl complete: %d found, %d new, %d updated",
            events_found,
            events_new,
            events_updated,
        )
        ...
        logger.error("Failed to crawl Fulton County meetings: %s", exc)
        raise
