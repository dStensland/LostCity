"""
Crawler for Atlanta Public Schools Adult Education (atlantapublicschools.us).

APS Adult Education provides educational programs for adults:
...
Headquartered in downtown Atlanta, serves adult learners across the district.

STRATEGY:
- Scrape adult education section for program events
- Extract GED orientations, ESL registration, career workshops
- Tag appropriately: education, ged, esl, literacy, free, adult-education
- Most events are free public services
- Category: "learning" for all educational events
"""
...
VENUE_DATA = {
    ...
    "website": ADULT_ED_URL,
    "vibes": ["education", "ged", "esl", "literacy"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["education", "adult-education"]

    # GED programs
    if any(word in text for word in ["ged", "general education development", "high school equivalency"]):
        tags.extend(["ged", "literacy"])

    # HiSET programs
    if "hiset" in text or "high school equivalency test" in text:
        tags.extend(["ged", "literacy"])

    # ESL programs
    if any(word in text for word in ["esl", "english as a second language", "english language", "esol"]):
        tags.extend(["esl", "language"])

    # Literacy programs
    if any(word in text for word in ["literacy", "reading", "writing"]):
        tags.append("literacy")

    # Career readiness
    if any(word in text for word in ["career", "job readiness", "workforce", "employment"]):
        tags.extend(["career", "workforce"])

    # Digital literacy
    if any(word in text for word in ["digital", "computer", "technology", "online"]):
        tags.extend(["digital-literacy", "technology"])

    # Orientations and registration
    if any(word in text for word in ["orientation", "registration", "enrollment", "info session"]):
...
def try_simple_requests_first(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|calendar|date|program|class', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Atlanta Public Schools Adult Education events.
    ...
    try:
        ...
        # Try multiple pages
        events_pages = [
            f"{ADULT_ED_URL}/events",
            f"{ADULT_ED_URL}/calendar",
            f"{ADULT_ED_URL}/programs",
            ADULT_ED_URL,
            f"{BASE_URL}/domain/4527",  # Adult ed domain
        ]
        ...
        if not soup:
            logger.info(f"Fetching with Playwright: {ADULT_ED_URL}")
            with sync_playwright() as p:
                ...
                page.goto(ADULT_ED_URL, wait_until="domcontentloaded", timeout=30000)
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"APS Adult Education venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                skip_patterns = ["hours", "contact", "about", "staff", "location", "home"]
                ...
                event_record = {
                    ...
                    "price_min": None,
                    "price_max": None,
                    "price_note": "Free adult education programs",
                    "is_free": is_free,
        ...
        logger.info(
            f"APS Adult Education crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching APS Adult Education events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl APS Adult Education: {e}")
        raise
