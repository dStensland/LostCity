"""
Crawler for Cobb County public line dancing classes.

Targets the official Rec1 line dancing group and expands the county's
...
TARGET_TAB_ID = "8572"
TARGET_GROUP = "Adrian's Feet-N-Motion Soul LINE DANCING"
WEEKS_AHEAD = 10
...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    if location != "south cobb recreation center":
        return None
    ...
    description = (
        f"Public line dancing class at {VENUE_DATA['name']} through Cobb County Parks. "
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not checkout_key:
        logger.error("Cobb line dancing crawl aborted: missing checkout key")
        return 0, 0, 0

    tabs = _get_tabs(TENANT_SLUG, checkout_key)
    if not any(str(tab.get("id")) == TARGET_TAB_ID for tab in tabs):
        logger.error("Cobb line dancing crawl aborted: target tab missing")
        return 0, 0, 0
    ...
    if not target_group:
        logger.error("Cobb line dancing crawl aborted: target group missing")
        return 0, 0, 0
    ...
    for session in sessions:
        ...
        for event_date, weekday in parsed["occurrences"]:
            ...
            event_record = {
                ...
                "is_all_day": False,
                "category": "dance",
                "subcategory": "dance.line-dancing",
                "tags": [
                    "line-dancing",
                    "dance-class",
                    "public-fitness",
    ...
    if stale_removed:
        logger.info("Removed %s stale Cobb line dancing rows", stale_removed)

    logger.info(
        "Cobb line dancing crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
