"""
Crawler for American Parkinson Disease Association Georgia (apdaparkinson.org/community/georgia).

APDA Georgia provides support, education, and resources for individuals with Parkinson's
disease and their families.

Events include:
- Monthly support groups (typically 2nd Thursday at 6pm)
- Optimism Walk
- Exercise classes (Pedaling for Parkinson's, Rock Steady Boxing)
- Educational seminars
- Caregiver workshops

Highly relevant for hospital portal (neurological care, family support, exercise programs).
"""
...
BASE_URL = "https://www.apdaparkinson.org"
EVENTS_URL = f"{BASE_URL}/community/georgia"

VENUE_DATA = {
    "name": "American Parkinson Disease Association Georgia",
    "slug": "apda-georgia",
    "address": "3400 Peachtree Rd NE",
    ...
    "website": BASE_URL,
    "vibes": ["parkinsons", "neurological", "support"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["exercise", "pedaling", "boxing", "dance", "yoga", "tai chi"]):
        return "wellness"

    if any(word in text for word in ["optimism walk", "walk", "run", "5k"]):
        return "fitness"
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    # Support groups and exercise classes are typically free
    if "support group" in text or "exercise class" in text:
        return True
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl APDA Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching APDA Georgia events: {EVENTS_URL}")
            ...
            for event_elem in events:
                try:
                    ...
                    content_hash = generate_content_hash(title, "APDA Georgia", start_date)
        ...
        logger.info(f"APDA Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated")

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching APDA Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl APDA Georgia: {e}")
        raise
