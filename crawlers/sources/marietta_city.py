"""
Crawler for City of Marietta Events Calendar (mariettaga.gov/Calendar.aspx).

Government events calendar for Marietta, GA - community events, festivals, parks & recreation,
...
VENUE_DATA = {
    ...
    "slug": "city-of-marietta",
    "address": "205 Lawrence St",
...
def categorize_event(title: str, description: str) -> tuple[str, list[str]]:
    ...
    if any(w in text for w in ["kids", "children", "family", "youth", "storytime"]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl City of Marietta calendar using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching City of Marietta calendar: {CALENDAR_URL}")
        ...
        logger.info(
            f"City of Marietta crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl City of Marietta: {e}")
        raise
