"""
Crawler for Atlanta Cancer Care Foundation (atlantacancercarefoundation.org).

Atlanta Cancer Care Foundation provides emergency financial assistance to cancer
patients in the 17-county metro Atlanta area. They help with basic living expenses
(rent, utilities, transportation) while patients focus on treatment.

While primarily a financial assistance program rather than events-based,
they may host:
- Community awareness events
- Fundraising programs
- Educational sessions about financial resources

Relevant for hospital patients navigating financial challenges during cancer treatment.

STRATEGY:
- Scrape the events/news page
- Extract fundraising events, awareness programs, community education
- Tag with: cancer, financial-assistance, patient-support, community
- Most events likely free or fundraisers
- Category: "community" for most events, "learning" for education
"""
...
BASE_URL = "https://atlantacancercarefoundation.org"
EVENTS_URL = f"{BASE_URL}/events"

VENUE_DATA = {
    "name": "Atlanta Cancer Care Foundation",
    "slug": "atlanta-cancer-care-foundation",
    "address": "5670 Peachtree Dunwoody Rd NE, Suite 1000",
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    tags = ["cancer", "patient-support"]

    # Fundraising events
    if any(word in text for word in ["fundraiser", "gala", "benefit", "golf tournament", "charity"]):
        category = "community"
        tags.extend(["fundraiser"])
        is_free = False

    # Educational sessions
    elif any(word in text for word in ["seminar", "workshop", "education", "lunch and learn", "financial resources"]):
        category = "learning"
        tags.extend(["patient-education", "financial-assistance"])
        is_free = True

    # Community awareness events
    elif any(word in text for word in ["awareness", "community event", "open house"]):
        category = "community"
        tags.extend(["awareness", "community"])
        is_free = True
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    try:
        ...
        if not events or len(events) == 0:
            ...
            logger.info(f"Atlanta Cancer Care Foundation venue record ensured (ID: {venue_id})")
            return 0, 0, 0
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(
                    title, "Atlanta Cancer Care Foundation", start_date
                )
        ...
        logger.info(
            f"Atlanta Cancer Care Foundation crawl complete: {events_found} found, "
