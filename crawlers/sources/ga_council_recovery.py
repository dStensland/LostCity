...
Events include:
- Recovery advocacy events and rallies
- Conference calendar and professional training
- Peer support specialist training and certification
- Community education workshops
- Policy advocacy and awareness campaigns

Service area: Statewide Georgia with headquarters in Buckhead, Atlanta.
...
def categorize_event(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    """
    Categorize Georgia Council events based on content.
    ...
    if any(kw in text for kw in [
        "peer support", "support group", "recovery support",
        "peer specialist", "peer certification", "recovery coaching"
    ]):
    ...
    if any(kw in text for kw in [
        "training", "certification", "ceu", "professional development",
        "credential", "ethics training", "cpss", "crps"
    ]):
    ...
    if any(kw in text for kw in [
        "conference", "summit", "symposium", "annual meeting",
        "statewide gathering"
    ]):
    ...
    if any(kw in text for kw in [
        "advocacy", "awareness", "rally", "recovery month",
        "prevention", "policy", "legislative"
    ]):
    ...
    if any(kw in text for kw in [
        "workshop", "seminar", "class", "education",
        "learn", "presentation", "webinar"
    ]):
    ...
    # Community events
    if any(kw in text for kw in [
        "community", "celebration", "recovery celebration",
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Georgia Council on Substance Abuse events calendar.
    ...
    try:
        ...
        logger.info(
            f"GA Council crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except requests.RequestException as e:
        logger.error(f"Failed to fetch GA Council events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl GA Council: {e}")
        raise
