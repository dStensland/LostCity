"""
Crawler for Contrast Artisan Ales (contrastartisanales.com).

Brewery in Marietta with food trucks, live music, and community events.
...
BASE_URL = "https://www.contrastartisanales.com"
EVENTS_URL = f"{BASE_URL}/calendar"

VENUE_DATA = {
    "name": "Contrast Artisan Ales",
    "slug": "contrast-artisan-ales",
    "address": "60 Powder Springs St Suite L",
...
def determine_category(title: str, description: str = "") -> tuple[str, Optional[str], list[str]]:
    ...
    tags = ["brewery", "beer", "marietta", "marietta-square"]
    ...
    if any(w in text for w in ["food truck", "food", "pop-up", "bbq", "tacos", "pizza"]):
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Contrast Artisan Ales events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching Contrast Artisan Ales: {EVENTS_URL}")
            ...
            while i < len(lines):
                ...
                if date_match:
                    ...
                    content_hash = generate_content_hash(
                        title, "Contrast Artisan Ales", start_date
                    )
        ...
        logger.info(
            f"Contrast Artisan Ales crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except Exception as e:
        logger.error(f"Failed to crawl Contrast Artisan Ales: {e}")
        raise
