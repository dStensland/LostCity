"""
Crawler for Giving Kitchen (givingkitchen.org/events).

Giving Kitchen provides emergency assistance to food service workers through
financial support and stability services. They organize fundraising events and
community programs. Site uses JavaScript rendering - must use Playwright.
"""
...
BASE_URL = "https://givingkitchen.org"
EVENTS_URL = f"{BASE_URL}/events"

# Giving Kitchen HQ venue
GIVING_KITCHEN_HQ = {
    "name": "Giving Kitchen",
    "slug": "giving-kitchen",
    "address": "1425 Ellsworth Industrial Blvd NW",
...
def parse_event_date(date_text: str) -> Optional[dict]:
    """
    Parse various date formats from Giving Kitchen events.
...
def determine_category(title: str, description: str = "") -> str:
    ...
    # Giving Kitchen events are primarily community/fundraising
    if any(
        word in text
        for word in ["fundraiser", "gala", "benefit", "auction", "campaign"]
    ):
        return "community"
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    if any(
        word in text
        for word in ["$", "ticket", "registration fee", "cost:", "price:", "donation"]
    ):
        return False

    # Default to False for fundraising org
    return False


def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Giving Kitchen events using Playwright.
    ...
    try:
        with sync_playwright() as p:
            ...
            # Get venue ID for Giving Kitchen HQ
            venue_id = get_or_create_place(GIVING_KITCHEN_HQ)

            logger.info(f"Fetching Giving Kitchen events: {EVENTS_URL}")
            ...
            # Common patterns: .event, .event-item, article, [data-event]
            event_selectors = [
                ".event-item",
                ".event",
                "article",
                    ...
                    if date_data:
                        ...
                        if title:
                            ...
                            content_hash = generate_content_hash(
                                title, "Giving Kitchen", date_data["start_date"]
                            )
                ...
                for event_el in events:
                    try:
                        ...
                        if title and start_date:
                            ...
                            content_hash = generate_content_hash(
                                title, "Giving Kitchen", start_date
                            )
        ...
        logger.info(
            f"Giving Kitchen crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Giving Kitchen: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Giving Kitchen: {e}")
        raise
