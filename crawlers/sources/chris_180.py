"""
Crawler for Chris 180 (chris180.org).

Formerly Chris Kids, providing trauma-informed care for children, youth, and families
...
BASE_URL = "https://chris180.org"
EVENTS_URL = f"{BASE_URL}/get-involved/events/"

VENUE_DATA = {
    "name": "Chris 180",
    "slug": "chris-180",
    "address": "1100 Hemphill Ave NW",
    ...
    "website": BASE_URL,
    "vibes": ["inclusive", "family-friendly"],
}
...
# Gala/ball fundraiser indicators
GALA_KEYWORDS = [
    "gala",
    "ball",
    "christal",
...
def determine_category_and_tags(title: str, description: str) -> tuple[str, Optional[str], list[str]]:
    ...
    tags = ["mental-health", "youth", "nonprofit"]

    # Gala/ball
    if any(kw in text for kw in GALA_KEYWORDS):
        tags.append("charity")
    ...
    if any(kw in text for kw in TRAINING_KEYWORDS):
        tags.extend(["trauma", "training", "mental-health"])
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    try:
        ...
        logger.error(f"Failed to crawl Chris 180: {e}")
        raise
