"""
Crawler for Faith Alliance of Metro Atlanta (faithallianceofmetroatlanta.org).

FAMA organizes interfaith dialogue, community service, and social justice
events across metro Atlanta. Site may use JavaScript rendering - uses Playwright.
"""
...
BASE_URL = "https://faithallianceofmetroatlanta.org"
EVENTS_URL = f"{BASE_URL}/events"

# FAMA headquarters venue
FAMA_HQ = {
...
def parse_date_string(date_str: str) -> Optional[dict]:
    """
    Parse various date formats found on FAMA events.
    Examples:
    - "February 15, 2026"
    - "Feb 15"
    - "2/15/2026"
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["workshop", "class", "training", "seminar", "education"]):
        return "education"
    if any(word in text for word in ["worship", "service", "prayer", "meditation"]):
        return "community"
    if any(word in text for word in ["film", "movie", "screening"]):
        return "film"
    if any(word in text for word in ["music", "concert", "choir"]):
        return "music"
    if any(word in text for word in ["volunteer", "service project", "cleanup"]):
        return "community"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["interfaith", "faith", "community"]

    if any(word in text for word in ["justice", "social justice", "advocacy"]):
        tags.append("social-justice")
    if any(word in text for word in ["dialogue", "conversation", "discussion"]):
        tags.append("dialogue")
    if any(word in text for word in ["muslim", "islam", "islamic"]):
        tags.append("muslim")
    if any(word in text for word in ["jewish", "judaism", "synagogue"]):
        tags.append("jewish")
    if any(word in text for word in ["christian", "church"]):
        tags.append("christian")
    if any(word in text for word in ["buddhist", "buddhism"]):
        tags.append("buddhist")
    if any(word in text for word in ["hindu", "hinduism"]):
        tags.append("hindu")
    if any(word in text for word in ["volunteer", "service"]):
        tags.append("volunteer")
    if any(word in text for word in ["peace", "reconciliation"]):
        tags.append("peace")
    if any(word in text for word in ["climate", "environment"]):
        tags.append("environmental")
    if any(word in text for word in ["racial", "racism", "equity"]):
        tags.append("racial-justice")
    if any(word in text for word in ["youth", "young adult"]):
        tags.append("youth")

    # Check for free events
    if not any(word in text for word in ["$", "ticket", "cost", "fee"]) or "free" in text:
        tags.append("free")
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    # Default to True for FAMA (most events are free)
    return True


def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Faith Alliance of Metro Atlanta events using Playwright.

    The site structure may vary, so this crawler attempts to find event
    listings in common formats.
    """
    source_id = source["id"]
    events_found = 0
    events_new = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            # Get venue ID for FAMA HQ
            venue_id = get_or_create_venue(FAMA_HQ)

            logger.info(f"Fetching FAMA events: {EVENTS_URL}")
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to load /calendar, trying root: {e2}")
                    try:
                ...
                for elem in events_elements:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "Faith Alliance of Metro Atlanta", date_info["start_date"]
                        )
        ...
        logger.info(
            f"FAMA crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching FAMA: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl FAMA: {e}")
        raise
