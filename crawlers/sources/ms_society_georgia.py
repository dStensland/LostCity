"""
Crawler for National MS Society Georgia Chapter (nationalmssociety.org/chapters/GAA).

The National MS Society Georgia Chapter provides support, resources, and advocacy for
individuals living with multiple sclerosis and their families.

Events include:
- Walk MS Atlanta (April at Atlantic Station)
- Women's support groups
- Educational programs
- Wellness programs
- Community events

Highly relevant for hospital portal (neurological care, family support, wellness).
"""
...
BASE_URL = "https://www.nationalmssociety.org"
EVENTS_URL = f"{BASE_URL}/chapters/GAA/events"

VENUE_DATA = {
    "name": "National MS Society Georgia Chapter",
    "slug": "ms-society-georgia",
    "address": "455 Abernathy Rd NE, Suite 210",
    ...
    "website": f"{BASE_URL}/chapters/GAA",
    "vibes": ["multiple-sclerosis", "neurological", "support"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["support group", "women's support", "caregiver support"]):
        return "support_group"

    if any(word in text for word in ["walk ms", "walk", "run", "5k"]):
        return "fitness"

    if any(word in text for word in ["yoga", "wellness", "exercise", "fitness"]):
        return "wellness"

    if any(word in text for word in ["workshop", "seminar", "education", "lecture"]):
        return "learning"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["multiple-sclerosis", "neurological"]
    ...
    if "wellness" in text or "yoga" in text:
...
def is_free_event(title: str, description: str = "") -> bool:
    ...
    # Support groups and wellness programs are typically free
    if "support group" in text or "wellness" in text:
        return True
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl National MS Society Georgia events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching MS Society Georgia events: {EVENTS_URL}")
            ...
            for event_elem in events:
                try:
                    ...
                    content_hash = generate_content_hash(title, "MS Society Georgia", start_date)
        ...
        logger.info(f"MS Society Georgia crawl complete: {events_found} found, {events_new} new, {events_updated} updated")

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching MS Society Georgia: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl MS Society Georgia: {e}")
        raise
