"""
Crawler for Meals On Wheels Atlanta (mowama.org).

Meals On Wheels Atlanta provides meal delivery services to seniors and
organizes volunteer opportunities and community events. Site may use
...
BASE_URL = "https://www.mowama.org"
EVENTS_URL = f"{BASE_URL}/events"
VOLUNTEER_URL = f"{BASE_URL}/volunteer"

# Meals On Wheels Atlanta HQ venue
MOWA_HQ = {
    "name": "Meals On Wheels Atlanta",
    "slug": "meals-on-wheels-atlanta",
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["volunteer", "meal delivery", "pack", "driver", "kitchen"]):
        return "community"
    if any(word in text for word in ["fundraiser", "gala", "benefit", "donor"]):
        return "community"
    if any(word in text for word in ["workshop", "training", "seminar", "education"]):
        return "education"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    if any(word in text for word in ["senior", "elderly", "aging"]):
        tags.append("seniors")
    if any(word in text for word in ["food", "meal", "nutrition", "kitchen"]):
        tags.append("food")
...
def parse_date_from_text(text: str) -> Optional[str]:
    ...
    # Common patterns: "January 31, 2026", "Jan 31", "1/31/2026"
...
def crawl_url(page, url: str, source_id: int, venue_id: int) -> tuple[int, int, int]:
    """Crawl a specific URL for events."""
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        # Look for patterns like event titles followed by dates
        i = 0
        while i < len(lines):
            line = lines[i]

            # Skip short lines, navigation, and common non-event text
            if (len(line) < 10 or
                line.lower() in ["home", "about", "contact", "donate", "volunteer", "events", "news"] or
                line.startswith("http")):
                i += 1
                continue

            # Check if this could be an event title
            # Events usually have dates nearby
            ...
            if event_date:
                ...
                content_hash = generate_content_hash(
                    title, "Meals On Wheels Atlanta", event_date
                )
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl Meals On Wheels Atlanta events using Playwright.
    Checks both /events and /volunteer pages.
    """
    source_id = source["id"]
    total_found = 0
    total_new = 0
    ...
    try:
        with sync_playwright() as p:
            ...
            # Try volunteer page
            found, new, updated = crawl_url(page, VOLUNTEER_URL, source_id, venue_id)
            total_found += found
            total_new += new
            total_updated += updated
        ...
        logger.info(
            f"Meals On Wheels Atlanta crawl complete: {total_found} found, {total_new} new, {total_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Meals On Wheels Atlanta: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Meals On Wheels Atlanta: {e}")
        raise
