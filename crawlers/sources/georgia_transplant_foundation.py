"""
Crawler for Georgia Transplant Foundation (gatransplant.org).

Georgia Transplant Foundation provides support and advocacy for transplant
patients and their families throughout Georgia.

Key programs:
- Virtual support groups for transplant recipients and caregivers
- Southern Santé gala (annual fundraiser, typically March at The Carlyle)
- Patient assistance programs
- Educational workshops and symposiums
- Transplant awareness events
- Caregiver support programs

Located in Sandy Springs, serving transplant community statewide.

STRATEGY:
- Scrape events page for support groups, galas, and awareness events
- Tag: transplant, support-group, fundraiser, awareness
- Support groups are free; gala is ticketed fundraiser
- Category: "community" for support groups, galas; "learning" for educational events
"""
...
VENUE_DATA = {
    ...
    "website": BASE_URL,
    "vibes": ["transplant", "support", "community"],
}
...
def determine_category_and_tags(title: str, description: str = "") -> tuple[str, list[str], bool]:
    ...
    # Southern Santé gala (signature event)
    if "southern santé" in text or "sante" in text or "gala" in text:
        category = "community"
        tags.extend(["gala", "fundraiser", "formal"])
        is_free = False
    ...
    elif any(word in text for word in ["workshop", "seminar", "education", "training", "class"]):
        category = "learning"
        tags.extend(["education", "workshop", "free"])
        is_free = True

    # Awareness events
    elif "awareness" in text or "campaign" in text:
        category = "community"
        tags.extend(["awareness", "activism", "free"])
        is_free = True

    # Fundraising events (other than gala)
    elif "fundraiser" in text or "benefit" in text:
        category = "community"
...
def try_simple_requests(url: str) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        if soup.find(string=re.compile(r'event|support|gala', re.I)):
            return soup
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl Georgia Transplant Foundation events."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
        ...
        for event_elem in events:
            try:
                ...
                content_hash = generate_content_hash(title, "Georgia Transplant Foundation", start_date)
        ...
        logger.info(
            f"Georgia Transplant Foundation crawl complete: {events_found} found, "
            f"{events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching Georgia Transplant Foundation: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl Georgia Transplant Foundation: {e}")
        raise
