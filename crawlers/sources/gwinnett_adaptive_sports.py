...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    return {
        "title": f"{title} at {venue_data['name']}",
        "description": (
            f"Adaptive sports session at {venue_data['name']} through Gwinnett County Parks & Recreation. "
        ...
        "raw_text": f"{title} | {features.get('dates','')} | {features.get('times','')} | {venue_data['name']}",
    }
