"""
Crawler for DeKalb County Tobie Grant pickleball open play.

Targets the official Tobie Grant pickleball listing in DeKalb's ACTIVENet
catalog and expands the published Tuesday schedule into recurring open-play
instances.
"""
...
TARGET_NAME = "Pickle Ball at Tobie Grant"
MAX_PAGES = 25
WEEKS_AHEAD = 8
...
SCHEDULE_RE = re.compile(
    r"Every\s+(?P<days>Tuesdays?)\s*-\s*(?P<start>\d{1,2}(?::\d{2})?\s*(?:am|pm))\s*-\s*(?P<end>\d{1,2}(?::\d{2})?\s*(?:am|pm|noon))",
    re.IGNORECASE,
)

VENUE_DATA = {
    "name": "Tobie Grant Gymnasium",
    "slug": "tobie-grant-gymnasium",
    "address": "644 Parkdale Dr",
    ...
    "description": "DeKalb County gymnasium hosting public pickleball open play.",
}
...
def parse_item(item: dict, today: date) -> dict | None:
    ...
    title = f"Tobie Grant Pickleball Open Play at {VENUE_DATA['name']}"
    description = (
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not session or not csrf:
        logger.error("DeKalb Tobie Grant pickleball crawl aborted: session init failed")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info(
            "DeKalb Tobie Grant pickleball removed %d stale rows",
            stale_removed,
        )
