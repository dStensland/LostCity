"""
Crawler for GiGi's Playhouse Atlanta (gigisplayhouse.org/atlanta).

GiGi's Playhouse provides FREE educational and therapeutic programs for
individuals with Down syndrome and their families. Programs include:
- Literacy programs
- Therapy sessions
- Job training
- Summer camps
...
VENUE_DATA = {
    ...
    "website": BASE_URL,
    "vibes": ["down-syndrome", "disabilities", "family", "free", "education"],
}
...
def determine_category(title: str, description: str = "") -> str:
    ...
    if any(word in text for word in ["literacy", "reading", "book"]):
        return "learning"
    if any(word in text for word in ["therapy", "speech", "occupational"]):
        return "learning"
    if any(word in text for word in ["job", "employment", "career"]):
        return "learning"
    if any(word in text for word in ["camp", "summer"]):
        return "community"
    if any(word in text for word in ["social", "party", "dance"]):
        return "community"
    if any(word in text for word in ["fitness", "yoga", "dance", "exercise"]):
        return "fitness"
...
def extract_tags(title: str, description: str = "") -> list[str]:
    ...
    text = f"{title} {description}".lower()
    tags = ["down-syndrome", "disabilities", "free"]

    if any(word in text for word in ["literacy", "reading"]):
        tags.append("literacy")
    if any(word in text for word in ["therapy"]):
        tags.append("therapy")
    if any(word in text for word in ["job", "employment"]):
        tags.append("job-training")
    if any(word in text for word in ["camp"]):
        tags.append("camp")
    if any(word in text for word in ["family", "families"]):
        tags.append("family")
    if any(word in text for word in ["youth", "kids", "children", "teens"]):
        tags.append("youth")
...
def crawl(source: dict) -> tuple[int, int, int]:
    """Crawl GiGi's Playhouse Atlanta events using Playwright."""
    source_id = source["id"]
    events_found = 0
    events_new = 0
    events_updated = 0
    ...
    try:
                ...
                for event_elem in events:
                    try:
                        ...
                        content_hash = generate_content_hash(
                            title, "GiGi's Playhouse Atlanta", start_date
                        )
                        ...
                        if link_elem:
                            ...
                            if href:
                                if href.startswith("http"):
                                    ...
                                    event_url = BASE_URL.replace("/atlanta", "") + href
        ...
        logger.info(
            f"GiGi's Playhouse Atlanta crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching GiGi's Playhouse Atlanta: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl GiGi's Playhouse Atlanta: {e}")
        raise
