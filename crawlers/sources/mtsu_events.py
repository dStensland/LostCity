"""
Crawler for MTSU Events (mtsu.edu/calendar).
Middle Tennessee State University - campus events, performances, and lectures.
Uses Playwright for JavaScript-rendered calendar.
"""
...
def determine_category(text: str) -> str:
    ...
    if any(w in text_lower for w in ["concert", "music", "band", "recital"]):
        return "music"
    if any(w in text_lower for w in ["theater", "theatre", "play", "performance"]):
        return "theater"
    if any(w in text_lower for w in ["art", "gallery", "exhibit", "exhibition"]):
        return "art"
    if any(w in text_lower for w in ["sports", "game", "athletics"]):
        return "sports"
    if any(w in text_lower for w in ["lecture", "seminar", "workshop", "conference"]):
        return "education"
...
def crawl(source: dict) -> tuple[int, int, int]:
    """
    Crawl MTSU Events calendar using Playwright.
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(f"Fetching MTSU Events calendar: {CALENDAR_URL}")
            page.goto(CALENDAR_URL, wait_until="domcontentloaded", timeout=30000)
            page.wait_for_timeout(5000)  # Wait for calendar to render
            ...
            # Parse event listings - WordPress calendar might use .event, .tribe-event, etc.
            cards = page.query_selector_all("article, .event, .event-item, .tribe-event, .calendar-event")

            logger.info(f"Found {len(cards)} potential event cards on MTSU calendar")
            ...
            for card in cards:
                try:
                    ...
                    # Venue - could be a specific building on campus
                    venue_el = card.query_selector(".venue, .location, .event-venue, .tribe-venue, [itemprop='location']")
                    venue_name = venue_el.inner_text().strip() if venue_el else "MTSU Campus"

                    if venue_name and len(venue_name) > 2:
                        # If it looks like a building/room, append MTSU
                        if not "mtsu" in venue_name.lower() and not "middle tennessee" in venue_name.lower():
                            venue_name = f"{venue_name} - MTSU"
                    else:
                        venue_name = "MTSU Campus"
                    ...
                    event_record = {
                        ...
                        "category": category,
                        "subcategory": None,
                        "tags": ["mtsu", "university", "murfreesboro"],
                        "price_min": None,
                        "price_max": None,
                        "price_note": None,
                        "is_free": False,
        ...
        logger.info(
            f"MTSU Events crawl complete: {events_found} found, {events_new} new, {events_updated} updated"
        )

    except PlaywrightTimeout as e:
        logger.error(f"Timeout fetching MTSU Events: {e}")
        raise
    except Exception as e:
        logger.error(f"Failed to crawl MTSU Events: {e}")
        raise
