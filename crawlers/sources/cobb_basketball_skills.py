"""
Crawler for Cobb County public basketball skills classes.

Targets the official Rec1 basketball-skills group and expands the county's
...
TARGET_TAB_ID = "8572"
TARGET_GROUP = "Basketball Skills Training"
WEEKS_AHEAD = 10
...
PLACE_DATA = {
    ...
    "website": CATALOG_URL,
    "description": "Cobb County recreation center hosting public basketball skills classes.",
}
...
def parse_session(session: dict, today: date) -> dict | None:
    ...
    if location != "ward recreation center":
        return None
    ...
    description = build_program_description(
        title,
        summary="Public youth basketball skills class through Cobb County Parks.",
...
def crawl(source: dict) -> tuple[int, int, int]:
    ...
    if not checkout_key:
        logger.error("Cobb basketball skills crawl aborted: missing checkout key")
        return 0, 0, 0

    tabs = _get_tabs(TENANT_SLUG, checkout_key)
    if not any(str(tab.get("id")) == TARGET_TAB_ID for tab in tabs):
        logger.error("Cobb basketball skills crawl aborted: target tab missing")
        return 0, 0, 0
    ...
    if not target_group:
        logger.error("Cobb basketball skills crawl aborted: target group missing")
        return 0, 0, 0
    ...
    if stale_removed:
        logger.info("Removed %s stale Cobb basketball skills rows", stale_removed)

    logger.info(
        "Cobb basketball skills crawl complete: found=%s new=%s updated=%s",
        events_found,
        events_new,
        events_updated,
    )
