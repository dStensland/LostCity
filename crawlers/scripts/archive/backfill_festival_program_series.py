...
def _program_for_event(event: dict) -> str:
...
def _load_target_festivals(client, festival_slug: Optional[str]) -> Dict[str, dict]:
    if festival_slug:
        q = client.table("festivals").select("id,name,slug,categories,image_url").eq("slug", festival_slug).limit(1)
...
def _load_festival_program_series(client, festival_ids: List[str]) -> Dict[str, List[dict]]:
...
def backfill(execute: bool = False, festival_slug: Optional[str] = None) -> dict:
    ...
    for festival_id, series_rows in series_by_festival.items():
        ...
        for event in _iter_rows(event_query):
            ...
            program_title = _program_for_event(event)
            ...
            if cache_key not in program_series_cache:
                if execute:
                    ...
                    target_series_id = ensure_program_series(festivals[festival_id], program_title)
                    program_series_cache[cache_key] = target_series_id
                    if cache_key not in before_keys and cache_key not in existing_series_keys:
    ...
    for event_id, target_series_id in updates:
        client.table("events").update({"series_id": target_series_id}).eq("id", event_id).execute()
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Backfill per-program festival series assignment")
