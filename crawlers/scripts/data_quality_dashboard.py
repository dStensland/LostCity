...
def main():
    ...
    print(
        "║                    LOSTCITY DATA QUALITY DASHBOARD                         ║"
    )
    print(
        "╚════════════════════════════════════════════════════════════════════════════╝"
    )
    ...
    print("─" * 80)
    print(
        f"Tier 1 (Healthy):      {progress_bar(tier1_pct, 40)} {tier1}/{total_sources}"
    )
    print(
        f"Tier 2 (Warning):      {progress_bar(tier2_pct, 40)} {tier2}/{total_sources}"
    )
    print(
        f"Tier 3 (Stale):        {progress_bar(tier3_pct, 40)} {tier3}/{total_sources}"
    )
    print(
        f"Tier 4 (Never Active): {progress_bar(tier4_pct, 40)} {tier4}/{total_sources}"
    )
    print()
    ...
    print(
        f"║  OVERALL DATA QUALITY: {progress_bar(overall, 50)} {grade(overall):>4s}  ║"
    )
    print(
        "╚════════════════════════════════════════════════════════════════════════════╝"
    )
    print()
    print(
        f"Database: {total_events:,} events across {total_venues:,} venues from {total_sources} sources"
    )
    print()
