...
def fetch_all_events():
    ...
    q = (
        client.table("events")
        .select(
            "id,title,start_date,category_id,genres,tags,is_recurring,series_id,"
...
ACTIVITY_TYPES = {
    ...
    "nerd_stuff": {
        ...
        "title_re": re.compile(
            r"\bgame night\b|board game|d&d|dungeons|warhammer|magic.the.gathering",
            re.I,
        ),
    },
    "bar_games": {
        ...
        "title_re": re.compile(
            r"bowl|skee.?ball|darts|shuffleboard|bocce|cornhole|billiards|curling", re.I
        ),
    },
    "food_specials": {
        "genres": {"specials", "happy-hour", "wings", "taco", "burger", "crawfish"},
        "title_re": re.compile(
            r"happy hour|wing night|taco|burger night|crawfish", re.I
        ),
    },
    "brunch": {
...
def report_scene(scene_events, all_events):
    ...
    if unmatched:
        ...
        for e in unmatched[:15]:
            ...
            print(
                f'    - [{cat}] "{e["title"][:55]}" @ {e["venue_name"][:25]} | genres: {genres}'
            )
...
def report_lineup(lineup_events):
    ...
    has_desc = sum(
        1 for e in lineup_events if e.get("description") and len(e["description"]) > 50
    )
    has_ticket = sum(1 for e in lineup_events if e.get("ticket_url"))
    has_all = sum(
        1
        for e in lineup_events
        if e.get("image_url") and e.get("description") and e.get("ticket_url")
    )

    print("\n  Quality signals:")
    print(
        f"    Has image:       {has_image:>4} ({has_image/max(len(lineup_events),1)*100:.0f}%)"
    )
    print(
        f"    Has description: {has_desc:>4} ({has_desc/max(len(lineup_events),1)*100:.0f}%)"
    )
    print(
        f"    Has ticket URL:  {has_ticket:>4} ({has_ticket/max(len(lineup_events),1)*100:.0f}%)"
    )
    print(
        f"    Has all three:   {has_all:>4} ({has_all/max(len(lineup_events),1)*100:.0f}%)"
    )
    ...
    for i, e in enumerate(top, 1):
        ...
        cat = e.get("category_id", "")
        print(
            f'    {i:>2}. [{cat}] "{e["title"][:50]}" @ {e["venue_name"][:25]} (score={qs})'
        )

    # Check for recurring events that leaked into lineup
    recurring_in_lineup = [
        e for e in lineup_events if e.get("is_recurring") or e.get("series_id")
    ]
    print(f"\n  Recurring/series events in Lineup: {len(recurring_in_lineup)}")
    if recurring_in_lineup:
        print(
            "  (These are recurring events that passed through because they were in excluded categories or had premium tags)"
        )
        for e in recurring_in_lineup[:10]:
            ...
            print(
                f'    - [{cat}] ({recur}) "{e["title"][:45]}" | tags={tags} genres={genres}'
            )
...
def report_big_stuff(big_stuff_events):
    ...
    for e in sorted(big_stuff_events, key=lambda x: x["start_date"]):
        ...
        print(
            f'    [{e["start_date"]}] [{cat}] ({tent}) "{e["title"][:50]}" @ {e["venue_name"][:30]}'
        )
        if fid:
...
def report_summary(scene_events, lineup_events, big_stuff_events):
    ...
    set(e["id"] for e in scene_events + lineup_events + big_stuff_events)
...
def report_unaccounted(all_events, scene_ids, lineup_ids, big_stuff_ids):
    ...
    if unaccounted:
        ...
        film_series = sum(
            ...
            if e.get("category_id") == "film" and e.get("series_id")
        )
        unmatched_recurring = sum(
            1 for e in unaccounted if e.get("_unmatched_recurring")
        )
        ...
        if other > 0:
            others = [
                e
                for e in unaccounted
                if not e.get("is_class")
                and not e.get("is_sensitive")
            ...
            for e in others[:10]:
                ...
                recur = (
                    ...
                    else ("series" if e.get("series_id") else "one-off")
                )
                tags = e.get("tags") or []
                genres = e.get("genres") or []
                ...
                print(
                    f'      - [{cat}] ({recur}) "{e["title"][:50]}" @ {e["venue_name"][:25]}'
                )
                print(
                    f"        tags={tags}, genres={genres}, tentpole={tent}, festival={fest}"
                )
