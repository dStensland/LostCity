...
def get_all_venues():
    ...
    while True:
        response = (
            ...
            .select("id, name, lat, lng, city, venue_type")
            .range(offset, offset + page_size - 1)
            .execute()
        )
...
def run_audit():
    ...
    findings["garbage_titles"] = {
        "total": total_garbage,
        "severity": (
            "P0" if total_garbage > 50 else "P1" if total_garbage > 10 else "OK"
        ),
    ...
    findings["duplicates"] = {
        ...
        "severity": (
            "P0" if len(duplicates) > 20 else "P1" if len(duplicates) > 0 else "OK"
        ),
    }

    print(
        f"Duplicate groups: {len(duplicates)}, Extra events to delete: {total_dup_events}"
    )
    if duplicates:
        print("\nTop 10 duplicate groups:")
        for (title, venue_id, start_date), event_ids in sorted(
            duplicates.items(), key=lambda x: -len(x[1])
        )[:10]:
            print(
                f"  - '{title[:60]}' | venue={venue_id} | {start_date}: {len(event_ids)} copies"
            )
    ...
    permanent_attractions = {
        title: ids for title, ids in title_counts.items() if len(ids) > 20
    }
    ...
    findings["permanent_attractions"] = {
        ...
        "severity": (
            "P0"
            if len(permanent_attractions) > 5
            else "P1" if len(permanent_attractions) > 0 else "OK"
        ),
    }

    print(f"Titles appearing >20 times: {len(permanent_attractions)}")
    if permanent_attractions:
        for title, ids in sorted(
            permanent_attractions.items(), key=lambda x: -len(x[1])
        )[:10]:
    ...
    findings["past_events"] = {
        ...
        "severity": (
            "P1" if len(past_events) > 100 else "P2" if len(past_events) > 0 else "OK"
        ),
    }
    ...
    findings["far_future_events"] = {
        ...
        "severity": (
            "P1" if len(far_future) > 10 else "P2" if len(far_future) > 0 else "OK"
        ),
    }
    ...
    findings["missing_venue_data"] = {
        ...
        "severity": (
            ...
            else "P1" if len(missing_coords) > 0 else "OK"
        ),
    }
    ...
    orphaned = [
        e
        for e in all_events
        if e.get("source_id") and e["source_id"] not in active_source_ids
    ]

    findings["orphaned_events"] = {
        "total": len(orphaned),
        "severity": (
            "P1" if len(orphaned) > 100 else "P2" if len(orphaned) > 0 else "OK"
        ),
    }
    ...
    midnight_not_allday = [
        e
        for e in all_events
        if e.get("start_time") == "00:00:00" and not e.get("is_all_day")
    ]
    allday_with_time = [
        e
        for e in all_events
        if e.get("is_all_day") and e.get("start_time") and e["start_time"] != "00:00:00"
    ]
    synthetic_desc = [
        e
        for e in all_events
        if e.get("description") and re.match(r"^Event at .+$", e["description"].strip())
    ]
    ...
    sources_with_no_events = [
        s for s in active_sources if s["id"] not in source_future_counts
    ]
    ...
    summary = [
        (
            "Garbage titles",
            findings["garbage_titles"]["severity"],
            findings["garbage_titles"]["total"],
        ),
        (
            "Invalid categories",
            findings["invalid_categories"]["severity"],
            findings["invalid_categories"]["total"],
        ),
        (
            "NULL critical fields",
            findings["null_critical_fields"]["severity"],
            findings["null_critical_fields"]["total"],
        ),
        (
            "Duplicate events",
            findings["duplicates"]["severity"],
            f"{findings['duplicates']['duplicate_groups']} groups",
        ),
        (
            "Permanent attractions",
            findings["permanent_attractions"]["severity"],
            findings["permanent_attractions"]["high_frequency_titles"],
        ),
        (
            "Past events",
            findings["past_events"]["severity"],
            findings["past_events"]["total"],
        ),
        (
            "Far future events",
            findings["far_future_events"]["severity"],
            findings["far_future_events"]["total"],
        ),
        (
            "Venues missing coords",
            findings["missing_venue_data"]["severity"],
            findings["missing_venue_data"]["missing_coords"],
        ),
        (
            "Orphaned events",
            findings["orphaned_events"]["severity"],
            findings["orphaned_events"]["total"],
        ),
        (
            "Midnight not all-day",
            findings["suspicious_patterns"]["severity"],
            findings["suspicious_patterns"]["midnight_not_allday"],
        ),
        (
            "Sources with no events",
            findings["source_health"]["severity"],
            findings["source_health"]["sources_with_no_events"],
        ),
    ]
