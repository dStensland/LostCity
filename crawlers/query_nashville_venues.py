...
print("-" * 80)
result = client.table('places').select('name, neighborhood, description').contains('vibes', ['award-winning']).eq('city', 'Nashville').execute()
...
print("-" * 80)
result = client.table('places').select('name, address, vibes').contains('vibes', ['casual']).eq('neighborhood', 'East Nashville').eq('city', 'Nashville').execute()
...
result = client.table('places').select('name, neighborhood, vibes').eq('city', 'Nashville').execute()
...
print("-" * 80)
result = client.table('places').select('name, address, description').contains('vibes', ['tasting-menu']).eq('city', 'Nashville').execute()
for venue in result.data:
    print(f"   {venue['name']}")
...
print("\n5. Pizza Restaurants:")
print("-" * 80)
result = client.table('places').select('name, neighborhood, address').contains('vibes', ['pizza']).eq('city', 'Nashville').execute()
for venue in result.data:
    print(f"   {venue['name']} ({venue.get('neighborhood', 'N/A')})")
    print(f"      {venue.get('address', 'N/A')}")
...
print("\n6. Late-Night Venues:")
print("-" * 80)
result = client.table('places').select('name, neighborhood, description').contains('vibes', ['late-night']).eq('city', 'Nashville').execute()
for venue in result.data:
    print(f"   {venue['name']} ({venue.get('neighborhood', 'N/A')})")
