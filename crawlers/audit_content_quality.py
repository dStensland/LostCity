...
# ─────────────────────────────────────────────────────────────
from description_quality import AUDIT_GENERIC_RE as GENERIC_RE

ENCODING_RE = re.compile(
    r"&(?:amp|lt|gt|quot|#\d+|#x[\da-f]+);|</?[a-z][^>]*>", re.IGNORECASE
)
...
def audit_descriptions(events):
    ...
    title_as_desc = [
        e
        for e in has_desc
        if e["description"].strip().lower() == e["title"].strip().lower()
    ]
    ...
    for d, c in sorted(dupes.items(), key=lambda x: -x[1])[:10]:
        src = next(
            e["sources"]["name"] for e in has_desc if e["description"].strip() == d
        )
        print(
            f'    {c}x [{src}]: "{d[:80]}..."'
            if len(d) > 80
...
def audit_titles(events):
    ...
    allcaps = [
        e for e in events if e["title"] == e["title"].upper() and len(e["title"]) > 5
    ]
...
def audit_categories(events):
    ...
    mismatch_by_cat = Counter(
        f"{m[1]} in title, cat={m[0]['category_id']}" for m in mismatches
    )
    for pattern, cnt in mismatch_by_cat.most_common(15):
        print(f"    {cnt:>4}x  {pattern}")
    for e, kw, valid in mismatches[:5]:
        print(
            f"    EX: [{e['sources']['name']}] \"{e['title'][:60]}\" cat={e['category_id']} (expected {valid})"
        )
    ...
    print(
        f"  Music events: {len(music)}, without genre tags: {len(no_genre)} ({100*len(no_genre)/max(1,len(music)):.1f}%)"
    )
...
def audit_urls(events):
    ...
    print(
        f"  Listing page URLs (not detail): {len(listing_urls)} ({100*len(listing_urls)/max(1,len(has_url)):.1f}%)"
    )
...
def audit_images(events):
    ...
    for cat in cats:
        ...
        bar = "#" * int(pct / 5)
        print(
            f"  {cat:>20}: {len(cat_img):>4}/{len(cat_events):<4} ({pct:5.1f}%) {bar}"
        )
...
def audit_prices(events):
    ...
    unknown = [
        e
        for e in events
        if not e.get("is_free")
        and e.get("price_min") is None
        and e.get("price_max") is None
    ]
    ...
    for e in has_price:
        ...
        if p == 0:
            ...
            price_buckets["$1-20"] += 1
        elif p <= 50:
            price_buckets["$21-50"] += 1
        elif p <= 100:
            price_buckets["$51-100"] += 1
        else:
            price_buckets["$100+"] += 1
...
def audit_times(events):
    ...
    print(
        f"  Suspicious 00:00 times: {len(midnight)} ({100*len(midnight)/max(1,len(has_time)):.1f}%)"
    )
...
def rank_sources(events):
    ...
    for e in events:
        ...
        if not e.get("description"):
            s["no_desc"] += 1
        elif len(e["description"]) < 50:
            s["short_desc"] += 1
        elif TRUNCATION_RE.search(e["description"]):
            s["truncated"] += 1
        if not e.get("image_url"):
            s["no_img"] += 1
        if not e.get("start_time"):
            s["no_time"] += 1
        if e["title"] == e["title"].upper() and len(e["title"]) > 5:
            s["allcaps"] += 1
        if e.get("source_url") and LISTING_PATTERNS.search(e["source_url"]):
            s["listing_url"] += 1
    ...
    for src, score, total, s in rankings[:25]:
        print(
            f"  {src:<38} {score:>6} {total:>7} {s['no_desc']:>6} {s['no_img']:>6} {s['no_time']:>6} {s['allcaps']:>5} {s['listing_url']:>5}"
        )
