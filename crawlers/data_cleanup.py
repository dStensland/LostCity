...
def main():
    ...
    try:
        ...
        old_result = (
            ...
            .lt("start_date", cutoff_date)
            .execute()
        )
        old_count = old_result.count if hasattr(old_result, "count") else 0

        if old_count > 0:
            client.table("events").delete().lt("start_date", cutoff_date).execute()
    ...
    try:
        ...
        if meetup_count > 0:
            client.table("events").delete().eq("source_id", 2).execute()
            print(f"   ✓ Deleted {meetup_count:,} Meetup events")
        else:
            print("   ✓ No Meetup events to delete")
    except Exception as e:
        print(f"   ✗ Error: {e}")
    ...
    try:
        ...
        if generic_count > 0:
            client.table("events").delete().eq("source_id", 319).execute()
            print(f"   ✓ Deactivated source and deleted {generic_count:,} events")
        else:
            print("   ✓ Source deactivated, no events to delete")
    except Exception as e:
        print(f"   ✗ Error: {e}")
    ...
    try:
        ...
        if not landmark_events.data:
            print("   ✓ No events found at venue_id 199")
        else:
            seen = {}
            ...
            if duplicates_to_delete:
                ...
                print(
                    f"   ✓ Deleted {total_deleted:,} duplicate events (kept {len(seen):,} unique events)"
                )
            else:
    ...
    try:
        ...
        if source_counts:
            print("   Top sources missing source_url (from sample of 1000):")
            ...
            print("   ✓ All events have source_url!")
    except Exception as e:
        print(f"   ✗ Error: {e}")
