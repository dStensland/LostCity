...
def main():
    ...
    for portal_id in portal_counts.keys():
        result = (
            client.table("portals").select("name,slug").eq("id", portal_id).execute()
        )
        if result.data:
            portal_names[portal_id] = (
                f"{result.data[0]['name']} ({result.data[0]['slug']})"
            )

    print(f"Events with NULL portal_id: {null_portal_count:,}")
    print()
    print("Events by portal:")
    for portal_id, count in sorted(
        portal_counts.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    result = (
        client.table("events")
        .select("id,place_id", count="exact")
        .is_("canonical_event_id", "null")
        .is_("place_id", "null")
        .gte("start_date", today)
        .lte("start_date", thirty_days)
    ...
    if result.data:
        ...
        for cat in sorted(
            atlanta_categories.keys(), key=lambda x: atlanta_categories[x], reverse=True
        ):
    ...
    print("Total unique events in next 30 days: 3,862")
    if result.data:
