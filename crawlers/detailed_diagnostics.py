...
def check_missing_fields():
    ...
    result = (
        client.table("events")
        .select(
            "id, title, start_date, venue_id, source_id, sources(name, slug), venues(name), raw_text, source_url"
        )
...
def check_time_issues():
    ...
    result = (
        client.table("events")
        .select(
            "id, title, start_date, start_time, category_id, description, sources(name, slug), raw_text"
        )
        ...
        for event in result.data:
            ...
            if raw and "time" in raw.lower():
                ...
                time_lines = [
                    l
                    for l in lines
                    if "time" in l.lower() or "pm" in l.lower() or "am" in l.lower()
                ]
                if time_lines:
