from scripts.seed_atlanta_activity_overlays_wave10_destinations import (
    ATLANTA_ACTIVITY_OVERLAYS_WAVE10,
    ENSURE_VENUES,
)


def test_wave10_destinations_targets_match_expected_venues() -> None:
    assert sorted(ATLANTA_ACTIVITY_OVERLAYS_WAVE10.keys()) == [
        "great-wolf-lodge-georgia",
        "main-event-atlanta",
        "stars-and-strikes-woodstock",
    ]
    assert sorted(ENSURE_VENUES.keys()) == sorted(ATLANTA_ACTIVITY_OVERLAYS_WAVE10.keys())


def test_wave10_destinations_features_have_required_fields_and_unique_slugs() -> None:
