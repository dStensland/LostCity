...
class DataAudit:
    ...
    def load_data(self, limit: Optional[int] = None):
        ...
        query = (
            self.client.table("events").select("*").gte("start_date", str(date.today()))
        )
        if limit:
    ...
    def find_duplicates(self):
            ...
            if event.get("venue_id") and event.get("start_date"):
                ...
                if venue:
                    content_hash = generate_content_hash(
                        event["title"], venue["name"], event["start_date"]
                    )
            ...
            if len(events) > 1:
                ...
                for i in range(1, len(events)):
                    self.duplicates.append(
                        (events[0]["id"], events[i]["id"], "exact_hash")
                    )
        ...
        for (event_date, venue_id), events in date_venue_map.items():
            if len(events) > 1:
                # Compare each pair
    ...
    def check_missing_data(self):
                ...
                self.missing_descriptions.append(
                    {
                        "id": event["id"],
                        "title": event["title"],
                        "source": self.sources.get(event.get("source_id"), {}).get(
                            "name", "Unknown"
                        ),
        ...
        print(
            f"  {len(self.missing_descriptions)} events with missing/poor descriptions"
        )
