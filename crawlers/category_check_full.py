...
def main():
    ...
    result = client.table("events").select("id,category_id").gte("start_date", today).limit(20000).execute()
    ...
    null_count = sum(1 for e in result.data if e.get("category_id") is None)
