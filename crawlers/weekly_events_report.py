...
def main():
    ...
    print(f"\n{'='*80}")
    print("WEEKLY EVENTS REPORT: February 9-15, 2026")
    print(f"{'='*80}\n")
    ...
    if festivals:
        ...
        series_ids = list(
            set(evt.get("series_id") for evt in festivals if evt.get("series_id"))
        )
        series_result = (
            client.table("series")
        ...
        for series_id, series_events in by_series.items():
            ...
            # Show date range
            dates = [
                evt.get("start_date") for evt in series_events if evt.get("start_date")
            ]
            if dates:
    ...
    print(
        f"Events missing start_time:  {missing_time:4d} ({(missing_time/total_count)*100:5.1f}%)"
    )
    print(
        f"Events missing image:       {missing_image:4d} ({(missing_image/total_count)*100:5.1f}%)"
    )
    print(
        f"Events missing description: {missing_description:4d} ({(missing_description/total_count)*100:5.1f}%)"
    )
    print(
        f"Free events:                {free_events:4d} ({(free_events/total_count)*100:5.1f}%)"
    )
