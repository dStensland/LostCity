...
**IMPORTANT**: column names above are best-effort from the draft. Before the first real run, verify against the actual schema — if a column doesn't exist, log the mismatch and abort rather than filing garbage.
...
- Sources that already have `consecutive_failures ≥ 10` and are long-dead — those have stale issues already
