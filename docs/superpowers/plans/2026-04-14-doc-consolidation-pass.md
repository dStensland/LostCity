...
3. **`exhibition_type` enum expansion — ALREADY SHIPPED.** Verified against `20260410010002_exhibitions_expansion.sql` (2026-04-10). Enum accepts `solo`, `group`, `installation`, `retrospective`, `popup`, `permanent`, `seasonal`, `special-exhibit`, `attraction`. Non-arts portals should use `attraction`/`seasonal`/`special-exhibit`/`permanent`. Initial framing in earlier appendix drafts was stale — caught by round-2 architect-review and fixed. No follow-up needed. Further enum values may still be added later as usage patterns emerge.
