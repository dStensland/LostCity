...
export interface SearchFilters {
  ...
  time_after?: string;
  // Festival slug filter — filters events whose festival_id matches the given festival slug.
  // Populated from the ?series=<slug> URL param generated by CityBriefing active festival links.
  festival_slug?: string;
  // Audience filter — maps to audience_tags column on events.
  // "family" → overlaps(["toddler","preschool","kids","teen"])
  // "21plus" → overlaps(["21+"])
  // "general" or undefined → no audience_tags filter
  audience?: "family" | "21plus" | "general";
}
...
async function applySearchFilters(
  ...
  // Apply festival slug filter — look up festival by slug, then filter events by festival_id.
  // Used by the ?series=<festival-slug> URL param generated by CityBriefing's active festival link.
  if (filters.festival_slug) {
    const { data: festivalRow, error: festivalError } = await supabase
      .from("festivals")
      .select("id")
      .eq("slug", filters.festival_slug)
      .maybeSingle();

    if (festivalError) {
      logger.error("festival_slug lookup failed, skipping filter", festivalError);
    } else if (festivalRow) {
      query = query.eq("festival_id", (festivalRow as { id: string }).id);
    } else {
      // No matching festival — return nothing
      return { query, shouldReturnEmpty: true };
    }
  }
