...
export function mapEventServerDataToViewData(
  ...
  nearbyDestinations?: Record<string, Array<{ id: number; name: string; slug: string; venue_type: string | null; neighborhood: string | null; distance?: number; image_url: string | null; hours?: Record<string, { open: string; close: string } | null> | null; vibes?: string[] | null }>>
): EventApiResponse {
  ...
  const mappedEvent: EventApiResponse["event"] = {
    ...
    venue: event.venue
      ? {
          ...
          venue_type: event.venue.venue_type ?? null,
