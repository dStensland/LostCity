...
const SerendipityIcon = ({ type, className = "" }: { type: SerendipityType; className?: string }) => {
  const icons: Record<SerendipityType, { path: string }> = {
    hidden_gem: {
      // Diamond/gem shape
  ...
  return (
    <svg
      data-serendipity={type}
      className={`${className} icon-neon serendipity-icon`}
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="currentColor"
...
const SERENDIPITY_CONFIGS: Record<SerendipityType, {
  ...
  try_something_new: {
    ...
    // Subtle neutral gradient - doesn't compete with theme
    ...
    >
      ...
      >
        ...
        <Link
          ...
          className="block mx-3 mb-3 p-3 rounded-xl bg-[var(--card-bg)] border border-[var(--twilight)] hover:border-[var(--neon-magenta)]/30 transition-all group card-atmospheric serendipity-card glow-category"
        >
