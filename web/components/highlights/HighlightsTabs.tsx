...
export default function HighlightsTabs({ activePeriod, onChange }: HighlightsTabsProps) {
  return (
    <div className="highlights-tabs-shell flex gap-1 p-1 bg-[var(--night)] rounded-xl border border-[var(--twilight)]/30 mb-4">
      {TABS.map((tab) => (
        <button
          key={tab.key}
          onClick={() => onChange(tab.key)}
          className={`highlights-tab-btn flex-1 px-3 py-2 rounded-lg font-mono text-xs font-medium transition-all duration-200 ${
            activePeriod === tab.key
              ? "highlights-tab-active bg-[var(--twilight)] text-[var(--neon-amber)] border border-[var(--neon-amber)]/20"
