  ...
  if (variant === "compact") {
    return (
      <>
        ...
        <Link
          href={href}
          className={`flex items-center gap-3 p-3 rounded-lg border border-[var(--twilight)] bg-[var(--card-bg)] transition-all hover:bg-[var(--card-bg-hover)] hover:border-[var(--soft)] snap-start min-w-[280px] sm:min-w-0 related-card-accent ${
            accentClass?.className ?? ""
          }`}
        >
          {icon && (
            <div className="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center related-card-icon">
              {icon}
            </div>
          )}
