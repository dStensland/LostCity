...
export default function OrganizationAutocomplete({
    ...
    const searchOrganizations = async () => {
      ...
      try {
        const res = await fetch(
          `/api/organizations/search?q=${encodeURIComponent(debouncedQuery)}&limit=8&include_unverified=true`
        );
  ...
  const selectOrganization = useCallback((organization: OrganizationResult) => {
  ...
  const handleKeyDown = useCallback(
      ...
      switch (e.key) {
        ...
        case "Enter":
          ...
          if (highlightedIndex >= 0 && results[highlightedIndex]) {
            selectOrganization(results[highlightedIndex]);
          }
          break;
        case "Escape":
          setIsOpen(false);
    ...
    [isOpen, highlightedIndex, results, selectOrganization]
  );
                ...
                <div className="flex-1">
                  ...
                  {(organization.org_type || organization.neighborhood) && (
                    <div className="font-mono text-xs text-[var(--muted)] mt-0.5">
                      {[formatOrgType(organization.org_type), organization.neighborhood]
                        .filter(Boolean)
                        .join(" · ")}
                    </div>
                  )}
...
export { OrganizationAutocomplete as ProducerAutocomplete };
