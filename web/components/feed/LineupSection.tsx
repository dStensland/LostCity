...
export default function LineupSection({
  ...
  // Date tab counts — derived from the actual filtered event pools so the pill
  // number always matches what users see when they click the tab.
  // For tabs not yet fetched, fall back to server counts as a placeholder.
  const dateTabCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const tab of TABS) {
      const pool = tabEventPools[tab.id];
      if (pool) {
        counts[tab.id] = pool.length;
      } else {
        // Tab hasn't been fetched yet — use server count as placeholder
  ...
  }, [tabEventPools, tabCounts, lazyTabCounts]);
