...
export default function TasteProfilePanel() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push("/auth/login?redirect=/settings?tab=taste");
      return;
    }
  ...
  if (authLoading || loading) {
    return (
      <div className="animate-pulse space-y-6">
        ...
        <div className="h-40 rounded-xl bg-[var(--twilight)]" />
        <div className="h-40 rounded-xl bg-[var(--twilight)]" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-xl font-semibold text-[var(--cream)]">Taste Profile</h2>
        <p className="font-mono text-xs text-[var(--muted)] mt-1">
          What the system learned from your activity.
        </p>
      </div>
      ...
      <section className="grid grid-cols-3 gap-3">
        ...
        <div className="p-4 rounded-xl border border-[var(--twilight)] text-center bg-[var(--card-bg)]">
          <div className="text-2xl font-semibold text-[var(--lavender)]">
            {profile.stats.followedOrganizations ?? profile.stats.followedProducers ?? 0}
          </div>
          <div className="text-xs text-[var(--muted)] font-mono mt-1">Organizations</div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-[var(--twilight)] p-5 bg-[var(--card-bg)]">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-[var(--cream)]">Your Preferences</h3>
            <p className="text-xs text-[var(--muted)] mt-0.5">Categories and vibes you&apos;ve told us you like</p>
          </div>
          <Link href="/settings?tab=preferences" className="text-xs text-[var(--coral)] font-mono hover:underline">
            Edit
          </Link>
        </div>
        ...
        {profile.explicit.favorite_categories.length > 0 || profile.explicit.favorite_vibes.length > 0 ? (
          <div className="space-y-4">
            {profile.explicit.favorite_categories.length > 0 && (
              <div>
                <h4 className="text-xs text-[var(--muted)] font-mono uppercase tracking-wider mb-2">Categories</h4>
            ...
            {profile.explicit.favorite_vibes.length > 0 && (
              <div>
                <h4 className="text-xs text-[var(--muted)] font-mono uppercase tracking-wider mb-2">Vibes</h4>
                <div className="flex flex-wrap gap-2">
          ...
          <p className="text-sm text-[var(--muted)] text-center py-4">
            ...
            <Link href="/settings?tab=preferences" className="text-[var(--coral)] hover:underline">
              Add some
            </Link>
          </p>
        )}
      ...
      <section className="rounded-xl border border-[var(--twilight)] p-5 bg-[var(--card-bg)]">
        <div className="mb-4">
          <h3 className="font-semibold text-[var(--cream)]">Category Affinity</h3>
      ...
      {profile.stats.topNeighborhoods.length > 0 && (
        <section className="rounded-xl border border-[var(--twilight)] p-5 bg-[var(--card-bg)]">
          <div className="mb-4">
            <h3 className="font-semibold text-[var(--cream)]">Favorite Neighborhoods</h3>
      ...
      <section className="rounded-xl border border-[var(--twilight)] p-5 bg-[var(--card-bg)]">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-[var(--cream)]">Reset Learned Preferences</h3>
