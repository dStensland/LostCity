...
interface Props {
  ...
  slug: string; // the owner's username, used for back link and recommend URL
  group: { slug: string; name: string; description: string | null };
  entries: QueueEntry[];
  recommendations?: PublicRecommendation[];
}

export function GoblinGroupPublicView({ user, slug, group, entries, recommendations = [] }: Props) {
  ...
  // Debounced TMDB search (reuses queue-level search endpoint)
  ...
  // Trunk ring animation — identical to GoblinQueuePublicView
      ...
      <div className="max-w-2xl mx-auto px-4 py-12 sm:py-20 relative z-10">
        ...
        <div className="mb-10 pb-6 px-6 pt-6 rounded-lg bg-white/[0.03] border border-amber-900/20 shadow-[0_8px_32px_rgba(0,0,0,0.6)]">
          <div className="flex items-end gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1
                className="text-3xl sm:text-5xl font-black text-white uppercase tracking-[0.15em] leading-none break-words"
                style={{
                  textShadow:
          ...
          {group.description && (
            <p className="mt-4 text-sm text-zinc-400 italic max-w-xl font-mono leading-relaxed">
              {group.description}
            </p>
          )}
        ...
        >
          ...
          <p className="text-zinc-500 font-mono text-xs mb-6 tracking-wide">
            Send {displayName} a movie for the{" "}
            <span className="text-amber-500/90">{group.name}</span> group.
          </p>
...
export type { Props as GoblinGroupPublicViewProps };
