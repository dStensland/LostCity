...
export default async function Image({
  ...
  const titleSize = event.title.length > 50 ? "text-[44px]" : "text-[56px]";
      ...
      <div tw="relative flex flex-col w-full h-full bg-[#0D0D10]">
        ...
        <div tw="relative flex flex-col flex-1 justify-end p-[60px]">
          ...
          <div tw="flex items-center gap-4 mb-2">
            <span tw="text-[26px] font-semibold text-[#FF6B6B]">
              {formattedDate}
            </span>
            <span tw={`text-[24px] ${timeColor}`}>{formattedTime}</span>
            {event.is_free && (
              <span tw="text-[16px] font-semibold text-[#34D399] bg-[rgba(52,211,153,0.2)] px-2.5 py-1 rounded-md">
                FREE
              </span>
            )}
          </div>
          ...
          <div tw="flex items-center gap-2 mb-6">
            <span tw={`text-[22px] ${venueColor}`}>{venueName}</span>
            {neighborhood && (
              <span tw={`text-[18px] ${neighborhoodColor}`}>
                &bull; {neighborhood}
              </span>
            )}
          </div>
